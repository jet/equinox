...
module Log =
    ...
    let propEvents name (kvps: System.Collections.Generic.KeyValuePair<string, string> seq) (log: ILogger) =
        let items = seq {
            for kv in kvps do
                yield sprintf "{\"%s\": %s}" kv.Key kv.Value
        }
    ...
    module InternalMetrics =
        ...
        let dump (log: ILogger) =
            ...
            let logActivity name count lat =
                log.Information(
                    "{name}: {count:n0} requests; Average latency: {lat:n0}ms",
                    name,
                    count,
                    (if count = 0L then Double.NaN else float lat / float count)
                )
            ...
            let logPeriodicRate name count =
                log.Information("rp{name} {count:n0}", name, count)

            for uom, f in measures do
                let d = f duration in

                if d <> 0. then
                    logPeriodicRate uom (float totalCount / d |> int64)

[<RequireQualifiedAccess; NoEquality; NoComparison>]
type EsSyncResult =
    | Written (*EventStore.ClientAPI.*) of WriteResult
    | Conflict of actualVersion: int64

module private Write =
    /// Yields `EsSyncResult.Written` or `EsSyncResult.Conflict` to signify WrongExpectedVersion
    let private writeEventsAsync
        (log: ILogger)
        ...
        async {
            ...
            with :? EventStore.ClientAPI.Exceptions.WrongExpectedVersionException as ex ->
                log.Information(
                    ex,
                    "Ges TrySync WrongExpectedVersionException writing {EventTypes}, actual {ActualVersion}",
                    [| for x in events -> x.Type |],
                    ex.ActualVersion
                )
    ...
    let eventDataBytes events =
        let eventDataLen (x: EventData) =
            match x.Data, x.Metadata with
            | Log.BlobLen bytes, Log.BlobLen metaBytes -> bytes + metaBytes

        events |> Array.sumBy eventDataLen

    let private writeEventsLogged
        (conn: IEventStoreConnection)
        (streamName: string)
        (version: int64)
        (events: EventData[])
        ...
        async {
            let log =
                if (not << log.IsEnabled) Events.LogEventLevel.Debug then
                    log
                else
                    log |> Log.propEventData "Json" events

            let bytes, count = eventDataBytes events, events.Length
            let log = log |> Log.prop "bytes" bytes

            let writeLog =
                log |> Log.prop "stream" streamName |> Log.prop "expectedVersion" version |> Log.prop "count" count
            ...
            let resultLog, evt =
                ...
                | EsSyncResult.Conflict actualVersion, m -> log |> Log.prop "actualVersion" actualVersion, Log.WriteConflict m
            ...
            (resultLog |> Log.event evt)
                .Information(
                    "Ges{action:l} count={count} conflict={conflict}",
                    "Write",
                    events.Length,
                    match evt with
                    | Log.WriteConflict _ -> true
                    | _ -> false
                )
    ...
    let writeEvents (log: ILogger) retryPolicy (conn: IEventStoreConnection) (streamName: string) (version: int64) (events: EventData[]) : Async<EsSyncResult> =
...
module private Read =
    ...
    let (|ResolvedEventLen|) (x: ResolvedEvent) =
        match x.Event.Data, x.Event.Metadata with
        | Log.BlobLen bytes, Log.BlobLen metaBytes -> bytes + metaBytes
    ...
    let private loggedReadSlice conn streamName direction batchSize startPos (log: ILogger) : Async<StreamEventsSlice> = async {
        ...
        let bytes, count =
            slice.Events |> Array.sumBy (|ResolvedEventLen|), slice.Events.Length
        ...
        let log =
            if (not << log.IsEnabled) Events.LogEventLevel.Debug then
                ...
                log |> Log.propResolvedEvents "Json" slice.Events

        (log |> Log.prop "startPos" startPos |> Log.prop "bytes" bytes |> Log.event evt)
            .Information("Ges{action:l} count={count} version={version}", "Read", count, slice.LastEventNumber)

        return slice
    }

    let private readBatches
        (log: ILogger)
        (readSlice: int64 -> ILogger -> Async<StreamEventsSlice>)
    ...
    let resolvedEventBytes events =
        events |> Array.sumBy (|ResolvedEventLen|)
    ...
    let logBatchRead direction streamName t events batchSize version (log: ILogger) =
        ...
        let action =
            match direction with
            | Direction.Forward -> "LoadF"
            | Direction.Backward -> "LoadB"

        let evt = Log.Metric.Batch(direction, batches, reqMetric)

        (log |> Log.prop "bytes" bytes |> Log.event evt)
            .Information("Ges{action:l} stream={stream} count={count}/{batches} version={version}", action, streamName, count, batches, version)

    let loadForwardsFrom (log: ILogger) retryPolicy conn batchSize maxPermittedBatchReads streamName startPosition : Async<int64 * ResolvedEvent[]> = async {
        let mergeBatches (batches: AsyncSeq<int64 option * ResolvedEvent[]>) = async {
            ...
            let! (events: ResolvedEvent[]) =
                ...
                |> AsyncSeq.map (function
                    | None, events -> events
                    | Some _ as reportedVersion, events ->
                        versionFromStream <- reportedVersion
                        events)
                |> AsyncSeq.concatSeq
                |> AsyncSeq.toArrayAsync

            let version =
                match versionFromStream with
                | Some version -> version
                | None -> invalidOp "no version encountered in event batch stream"

            return version, events
        }

        let call pos =
            loggedReadSlice conn streamName Direction.Forward batchSize pos

        let retryingLoggingReadSlice pos =
            Log.withLoggedRetries retryPolicy "readAttempt" (call pos)

        let direction = Direction.Forward

        let log =
            log |> Log.prop "batchSize" batchSize |> Log.prop "direction" direction |> Log.prop "stream" streamName

        let batches: AsyncSeq<int64 option * ResolvedEvent[]> =
            readBatches log retryingLoggingReadSlice maxPermittedBatchReads startPosition
    ...
    let partitionPayloadFrom firstUsedEventNumber : ResolvedEvent[] -> int * int =
        let acc (tu, tr) (ResolvedEventLen bytes as y) =
            if y.Event.EventNumber < firstUsedEventNumber then
                tu, tr + bytes
            else
                tu + bytes, tr
    ...
    let loadBackwardsUntilCompactionOrStart
        (log: ILogger)
        retryPolicy
        conn
        batchSize
        maxPermittedBatchReads
        streamName
        (tryDecode, isOrigin)
        : Async<int64 * struct (ResolvedEvent * 'event voption)[]> =
        async {
            let mergeFromCompactionPointOrStartFromBackwardsStream
                (log: ILogger)
                (batchesBackward: AsyncSeq<int64 option * ResolvedEvent[]>)
                : Async<int64 * struct (ResolvedEvent * 'event voption)[]> =
                async {
                    ...
                    let! tempBackward =
                        ...
                        |> AsyncSeq.map (fun batch ->
                            match batch with
                            | None, events ->
                                lastBatch.Value <- Some events
                                events
                            | Some _ as reportedVersion, events ->
                                versionFromStream.Value <- reportedVersion
                                lastBatch.Value <- Some events
                                events
                        ...
                        |> AsyncSeq.takeWhileInclusive (function
                            | x, ValueSome e when isOrigin e ->
                                ...
                                | Some batch ->
                                    ...
                                    log.Information(
                                        "GesStop stream={stream} at={eventNumber} used={used} residual={residual}",
                                        streamName,
                                        x.Event.EventNumber,
                                        used,
                                        residual
                    ...
                    let eventsForward =
                        Array.Reverse(tempBackward)
                        tempBackward // sic - relatively cheap, in-place reverse of something we own

                    let version =
                        match versionFromStream.Value with
                        | Some version -> version
                        | None -> invalidOp "no version encountered in event batch stream"
            ...
            let call pos =
                loggedReadSlice conn streamName Direction.Backward batchSize pos
            ...
            let batchesBackward: AsyncSeq<int64 option * ResolvedEvent[]> =
                readBatches readlog retryingLoggingReadSlice maxPermittedBatchReads startPosition
...
type Position = {
    streamVersion: int64
    compactionEventNumber: int64 option
    batchCapacityLimit: int option
}
...
module Token =
                    ...
                    pos =
                        {
                            ...
                            compactionEventNumber = compactionEventNumber
                            batchCapacityLimit = batchCapacityLimit
                        }
                }
    ...
    let ofNonCompacting streamVersion : StreamToken = create None None streamVersion
    ...
    let (*private*) ofCompactionEventNumber compactedEventNumberOption unstoredEventsPending batchSize streamVersion : StreamToken =
        let batchCapacityLimit =
            batchCapacityLimit compactedEventNumberOption unstoredEventsPending batchSize streamVersion
...
type EventStoreContext(connection: EventStoreConnection, batchOptions: BatchOptions) =
    ...
    let tryIsResolvedEventEventType predicateOption =
        predicateOption |> Option.map isResolvedEventEventType

    let conn requireLeader =
        if requireLeader then
            connection.WriteConnection
        else
            connection.ReadConnection

    new(connection: EventStoreConnection, [<O; D null>] ?batchSize)
    ...
    member _.LoadBatched streamName requireLeader log (tryDecode, isCompactionEventType) : Async<StreamToken * 'event[]> = async {
        let! version, events =
            Read.loadForwardsFrom log connection.ReadRetryPolicy (conn requireLeader) batchOptions.BatchSize batchOptions.MaxBatches streamName 0L
        ...
        | Some isCompactionEvent ->
            ...
            | Some resolvedEvent ->
                return Token.ofCompactionResolvedEventAndVersion resolvedEvent batchOptions.BatchSize version, Array.chooseV tryDecode events
    }

    member _.LoadBackwardsStoppingAtCompactionEvent streamName requireLeader log (tryDecode, isOrigin) : Async<StreamToken * 'event[]> = async {
        let! version, events =
            Read.loadBackwardsUntilCompactionOrStart
                log
                connection.ReadRetryPolicy
                (conn requireLeader)
                batchOptions.BatchSize
                batchOptions.MaxBatches
                streamName
                (tryDecode, isOrigin)
        ...
        match
            ...
            |> Option.filter (function
                | _, ValueSome e -> isOrigin e
                | _ -> false)
        with
        | None -> return Token.ofUncompactedVersion batchOptions.BatchSize version, Array.chooseV ValueTuple.snd events
        | Some(resolvedEvent, _) ->
            return Token.ofCompactionResolvedEventAndVersion resolvedEvent batchOptions.BatchSize version, Array.chooseV ValueTuple.snd events
    }

    member _.Reload requireLeader streamName log (Token.Unpack token as streamToken) (tryDecode, isCompactionEventType) : Async<StreamToken * 'event[]> = async {
        let streamPosition = token.streamVersion + 1L

        let! version, events =
            Read.loadForwardsFrom log connection.ReadRetryPolicy (conn requireLeader) batchOptions.BatchSize batchOptions.MaxBatches streamName streamPosition
        ...
        | Some isCompactionEvent ->
            match
                events
                |> Array.tryFindBack (fun re ->
                    match tryDecode re with
                    | ValueSome e -> isCompactionEvent e
                    | _ -> false)
            with
    ...
    member _.TrySync
        log
        streamName
        (Token.Unpack token as streamToken)
        (events, encodedEvents: EventData array)
        isCompactionEventType
        : Async<GatewaySyncResult> =
        async {
            ...
            | EsSyncResult.Conflict actualVersion -> return GatewaySyncResult.ConflictUnknown(Token.ofNonCompacting actualVersion)
                ...
                let token =
                    ...
                    | Some isCompactionEvent ->
                        ...
                        | Some compactionEventIndex ->
                            Token.ofPreviousStreamVersionAndCompactionEventDataIndex
                                streamToken
...
type private Category<'event, 'state, 'context>(context: EventStoreContext, codec: FsCodec.IEventCodec<_, _, 'context>, ?access: AccessStrategy<'event, 'state>) =
    ...
    let tryDecode (e: ResolvedEvent) =
        e |> UnionEncoderAdapters.encodedEventOfResolvedEvent |> codec.TryDecode
    ...
    let isOrigin =
        match access with
        | None
        | Some AccessStrategy.LatestKnownEvent -> fun _ -> true
        | Some(AccessStrategy.RollingSnapshots(isValid, _)) -> isValid

    let loadAlgorithm streamName requireLeader log =
        let batched = context.LoadBatched streamName requireLeader log (tryDecode, None)

        let compacted =
            context.LoadBackwardsStoppingAtCompactionEvent streamName requireLeader log (tryDecode, isOrigin)
    ...
    member _.Load
        (
            ...
            requireLeader,
            log: ILogger
        ) : Async<struct (StreamToken * 'state)> =
    ...
    member _.TrySync<'context>
        ...
        async {
            ...
            let events =
                match access with
                | None
                | Some AccessStrategy.LatestKnownEvent -> events
                | Some(AccessStrategy.RollingSnapshots(_, compact)) ->
                    ...
                    if cc.IsCompactionDue then
                        Array.append events (fold state events |> compact |> Array.singleton)
                    else
                        events

            let encodedEvents: EventData[] =
                events |> Array.map (encode >> UnionEncoderAdapters.eventDataOfEncodedEvent)
            ...
            | GatewaySyncResult.ConflictUnknown _ ->
                return
                    SyncResult.Conflict(fun ct ->
                        load fold state (context.Reload true streamName log streamToken (tryDecode, compactionPredicate))
                        |> Async.startAsTask ct)
            | GatewaySyncResult.Written token' -> return SyncResult.Written(token', fold state (Seq.ofArray events))
        }
...
type private Folder<'event, 'state, 'context>(category: Category<'event, 'state, 'context>, fold: 'state -> 'event seq -> 'state, initial: 'state, ?readCache) =
    let batched log streamName requireLeader =
        category.Load(fold, initial, streamName, requireLeader, log)

    interface ICategory<'event, 'state, 'context> with
        member _.Load(log, _categoryName, _streamId, streamName, allowStale, requireLeader, ct) =
            match readCache with
            | None -> Async.startAsTask ct (batched log streamName requireLeader)
...
type EventStoreCategory<'event, 'state, 'context>(resolveInner, empty) =
    ...
    new(context: EventStoreContext, codec: FsCodec.IEventCodec<_, _, 'context>, fold, initial, [<O; D(null)>] ?caching, [<O; D(null)>] ?access)
    ...
    // e.g., A key benefit is that reads of streams more than a few pages long get completed in constant time after the initial load
     =
        ...
        let folder =
            Folder<'event, 'state, 'context>(inner, fold, initial, ?readCache = readCacheOption)

        let category: ICategory<_, _, 'context> =
            match caching with
            | None -> folder :> _
            | Some(CachingStrategy.SlidingWindow(cache, window)) -> Caching.applyCacheUpdatesWithSlidingExpiration cache null window folder Token.supersedes
            | Some(CachingStrategy.FixedTimeSpan(cache, period)) -> Caching.applyCacheUpdatesWithFixedTimeSpan cache null period folder Token.supersedes
            | Some(CachingStrategy.SlidingWindowPrefixed(cache, window, prefix)) ->
                Caching.applyCacheUpdatesWithSlidingExpiration cache prefix window folder Token.supersedes

        let resolveInner streamIds =
            struct (category, FsCodec.StreamName.Internal.ofCategoryAndStreamId streamIds, ValueNone)
...
module private Discovery =
    let buildDns np (f: DnsClusterSettingsBuilder -> DnsClusterSettingsBuilder) =
        ...
        |> fun s ->
            match np with
            | NodePreference.Random -> s.PreferRandomNode()
            | NodePreference.PreferSlave -> s.PreferFollowerNode()
            | _ -> s
        |> f
        |> fun s -> s.Build()
    ...
    let configureDns clusterDns maybeManagerPort (x: DnsClusterSettingsBuilder) =
        ...
        |> fun s ->
            match maybeManagerPort with
            | Some port -> s.SetClusterGossipPort(port)
            | None -> s

    let inline configureSeeded (seedEndpoints: System.Net.EndPoint[]) (x: GossipSeedClusterSettingsBuilder) = x.SetGossipSeedEndPoints(seedEndpoints)

    // converts a Discovery mode to a ClusterSettings or a Uri as appropriate
    let (|DiscoverViaUri|DiscoverViaGossip|): Discovery * NodePreference -> Choice<Uri, ClusterSettings> =
        function
...
type EventStoreConnector
    (
        ...
        reqRetries: int,
        [<O; D(null)>] ?log: Logger,
        [<O; D(null)>] ?heartbeatTimeout: TimeSpan,
        [<O; D(null)>] ?concurrentOperationsLimit,
        [<O; D(null)>] ?readRetryPolicy,
        [<O; D(null)>] ?writeRetryPolicy,
        [<O; D(null)>] ?gossipTimeout,
        [<O; D(null)>] ?clientConnectionTimeout,
...
// Facilitates arbitrary customization of settings that are not explicitly addressed herein and/or general post-processing of the configuration.
 =
    let connSettings node =
        ConnectionSettings
            .Create()
            .SetDefaultUserCredentials(SystemData.UserCredentials(username, password))
        ...
        |> fun s ->
            match concurrentOperationsLimit with
            | Some col -> s.LimitConcurrentOperationsTo(col)
            | None -> s // ES default: 5000
        |> fun s ->
            match heartbeatTimeout with
            | Some v -> s.SetHeartbeatTimeout v
            | None -> s // default: 1500 ms
        |> fun s ->
            match gossipTimeout with
            | Some v -> s.SetGossipTimeout v
            | None -> s // default: 1000 ms
        |> fun s ->
            match clientConnectionTimeout with
            | Some v -> s.WithConnectionTimeoutOf v
            | None -> s // default: 1000 ms
        |> fun s ->
            match log with
            | Some log -> log.Configure s
            | None -> s
        |> fun s ->
            match custom with
            | Some c -> c s
    ...
    member _.Connect
        ( // Name should be sufficient to uniquely identify this connection within a single app instance's logs
            name,
            discovery: Discovery,
            [<O; D null>] ?clusterNodePreference
        ) : Async<IEventStoreConnection> =
        async {
            if name = null then
                nullArg "name"
            ...
            | ConnectionStrategy.ClusterTwinPreferSlaveReads ->
                ...
                return
                    EventStoreConnection(
                        readConnection = slave,
                        writeConnection = master,
