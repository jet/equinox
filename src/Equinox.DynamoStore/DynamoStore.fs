...
module private InternalBody =
    let ofStreamAndEncoding (stream: MemoryStream option, encoding: int option) : InternalBody =
        ...
        {
            encoding = defaultArg encoding 0
            data = stream
        }

    let toStreamAndEncoding (encoded: InternalBody) =
        Option.ofObj encoded.data,
        match encoded.encoding with
        | 0 -> None
        | x -> Some x

    let bytes (x: InternalBody) =
        if x.data = null then 0 else int x.data.Length
...
type Event =
    ...
    static member Bytes(x: Event) =
        ...
        x.c.Length
        + InternalBody.bytes x.d
        + InternalBody.bytes x.m
        + len x.correlationId
        + len x.causationId
        + 20 (*t*)
        + 20 (*overhead*)
...
type Unfold =
    ...
    static member Bytes(x: Unfold) =
        x.c.Length + InternalBody.bytes x.d + InternalBody.bytes x.m + 50

module Unfold =
    let arrayBytes (xs: Unfold array) =
        match xs with
        | null -> 0
        | u -> Array.sumBy Unfold.Bytes u
...
module Batch =
    ...
    and [<NoEquality; NoComparison>] EventSchema = {
        ...
        d: MemoryStream option
        D: int option // D carries encoding, None -> 0 // required
        m: MemoryStream option
        M: int option // M carries encoding, None -> 0
        x: string option
        y: string option
    }
    ...
    and [<NoEquality; NoComparison>] UnfoldSchema = {
        ...
        m: MemoryStream option
        M: int option
    } // M carries encoding, None -> 0

    let private toEventSchema (x: Event) : EventSchema =
        let (d, D), (m, M) =
            InternalBody.toStreamAndEncoding x.d, InternalBody.toStreamAndEncoding x.m
    ...
    let private ofUnfoldSchema (x: UnfoldSchema) : Unfold = {
        ...
        t = x.t
        c = x.c
        d = InternalBody.ofStreamAndEncoding (x.d, x.D)
        m = InternalBody.ofStreamAndEncoding (x.m, x.M)
    }
    ...
    let ofSchema (x: Schema) : Batch =
        ...
        let events =
            ...
            |> Seq.mapi (fun i (c, e) ->
                let d, m =
                    InternalBody.ofStreamAndEncoding (e.d, e.D), InternalBody.ofStreamAndEncoding (e.m, e.M)
...

        {
            ...
            b = x.b
            i = x.i
            etag = Option.toObj x.etag
            n = x.n
            e = Seq.toArray events
            u = x.u |> Array.map ofUnfoldSchema
        }
    ...
    let bytesBase (x: Batch) =
        80 + x.p.Length + String.length x.etag + Event.arrayBytes x.e

    let bytesTotal (xs: Batch seq) =
        xs |> Seq.sumBy (fun x -> bytesBase x + bytesUnfolds x)
...
module EncodedBody =
    ...
    let private decodeBody (raw: InternalBody) : EncodedBody =
        raw.encoding,
        if raw.data = null then
            ReadOnlyMemory.Empty
        else
            raw.data.ToArray() |> ReadOnlyMemory
    ...
    let internal toInternal struct (encoding, encodedBody: ReadOnlyMemory<byte>) : InternalBody = {
        encoding = encoding
        data =
            match encodedBody with
            | d when d.IsEmpty -> null
            | d -> new MemoryStream(d.ToArray(), writable = false)
    }
...
module Log =
    ...
    let (|MetricEvent|_|) (logEvent: Serilog.Events.LogEvent) : Metric voption =
        ...
        match p with
        | Log.ScalarValue(:? Metric as e) -> ValueSome e
        | _ -> ValueNone

    /// Attach a property to the captured event record to hold the metric information
    let internal event (value: Metric) =
        Internal.Log.withScalarProperty PropertyTag value
    ...
    module InternalMetrics =
        ...
        module Stats =
            ...
            type internal Counters() =
                ...
                member _.Ingest(table, ms, ru) =
                    tables.GetOrAdd(table, create).Ingest(ms, ru)

                member _.Tables = tables.Keys

                member _.TryTable table =
            ...
            let inline private (|TableMsRu|) ({ table = t; interval = i; ru = ru }: Measurement) = t, int64 i.ElapsedMilliseconds, ru
            ...
            type LogSink() =
                ...
                interface Serilog.Core.ILogEventSink with
                    member _.Emit logEvent =
                        ...
                        | MetricEvent cm ->
                            match cm with
                            | Op((Operation.Tip | Operation.Tip404 | Operation.Tip304), TableMsRu m) -> epoch.Tip.Ingest m
        ...
        let dump (log: ILogger) =
            ...
            for table in stats |> Seq.collect (fun (_n, stat) -> stat.Tables) |> Seq.distinct |> Seq.sort do
                ...
                let logActivity name count ru lat =
                    let aru, ams =
                        (if count = 0L then Double.NaN else ru / float count), (if count = 0L then Double.NaN else float lat / float count)
                    ...
                    let rut =
                        ...
                        |> function
                            ...
                            | nameof res.Query
                            | nameof res.Prune ->
                                totalRRu <- totalRRu + ru
                                "R"
                            | _ ->
                                totalWRu <- totalWRu + ru
                                "W"
                ...
                if rows > 1 then
                    logActivity "TOTAL" totalCount (totalRRu + totalWRu) totalMs

                let measures: (string * (TimeSpan -> float)) list = [
                    "s", (fun x -> x.TotalSeconds (*; "m", fun x -> x.TotalMinutes; "h", fun x -> x.TotalHours*) )
                ]

                let logPeriodicRate name count rru wru =
                    log.Information("{table} {rru:n1}R/{wru:n1}W CU @ {count:n0} rp{unit}", table, rru, wru, count, name)

                for uom, f in measures do
                    let d = f res.Elapsed in

                    if d <> 0. then
                        logPeriodicRate uom (float totalCount / d |> int64) (totalRRu / d) (totalWRu / d)
...
module Initialization =
    ...
    let private prepare (client: IAmazonDynamoDB) tableName maybeThroughput : Async<unit> =
        ...
        match maybeThroughput with
        | Some throughput -> context.VerifyOrCreateTableAsync(throughput) |> Async.Ignore
        | None -> context.VerifyTableAsync()
...
type Container(tableName, createContext: (RequestMetrics -> unit) -> TableContext<Batch.Schema>) =
    ...
    static member Create(client, tableName) =
        let createContext collector =
            TableContext<Batch.Schema>(client, tableName, metricsCollector = collector)

        Container(tableName, createContext)

    member x.TryGetTip(stream: string, consistentRead) : Async<Batch option * RequestConsumption> = async {
        let rm = Metrics()
        let context = createContext rm.Add
        let pk = Batch.tableKeyForStreamTip stream
        let! item = context.TryGetItemAsync(pk, consistentRead)
    ...
    member _.QueryBatches(stream, consistentRead, minN, maxI, backwards, batchSize) : AsyncSeq<int * StopwatchInterval * Batch array * RequestConsumption> =
        ...
        let rec aux (i, le) = asyncSeq {
            ...
            let! t, res =
                context.QueryPaginatedAsync(
                    kc,
                    ?filterCondition = fc,
                    limit = batchSize,
                    ?exclusiveStartKey = le,
                    scanIndexForward = not backwards,
    ...
    member internal _.QueryIAndNOrderByNAscending(stream, maxItems) : AsyncSeq<int * StopwatchInterval * BatchIndices array * RequestConsumption> =
        let rec aux (index, lastEvaluated) = asyncSeq {
            ...
            let! t, res =
                context.QueryProjectedPaginatedAsync(keyCond, proj, ?exclusiveStartKey = lastEvaluated, scanIndexForward = true, limit = maxItems)
                |> Stopwatch.Time
...
type Position =
    ...
    override x.ToString() =
        sprintf "{ n=%d; etag=%s; e=%d; b=%d+%d }" x.index x.etag x.events.Length x.baseBytes x.unfoldsBytes
...
module internal Position =
    ...
    let fromTip (x: Batch) = {
        ...
        etag = x.etag
        events = x.e
        calvedBytes = defaultArg x.b 0
        baseBytes = Batch.bytesBase x
        unfoldsBytes = Batch.bytesUnfolds x
    }

    let fromElements (p, b, n, e, u, etag) =
        fromTip
            {
                p = p
                b = Some b
                i = Unchecked.defaultof<_>
                n = n
                e = e
                u = u
                etag = etag
    ...
    let tryFromBatch (x: Batch) =
        if Batch.isTip x.i then fromTip x |> Some else None
    ...
    let toVersionAndStreamBytes =
        function
        | Some p -> p.index, p.calvedBytes + p.baseBytes
        | None -> 0, 0
...
module internal Sync =
    ...
    let private (|DynamoDbConflict|_|): exn -> _ =
        function
        | Precondition.CheckFailed
        | TransactWriteItemsRequest.TransactionCanceledConditionalCheckFailed -> Some()
        | _ -> None

    let private cce: Quotations.Expr<Batch.Schema -> bool> -> ConditionExpression<Batch.Schema> =
    ...
    let private putItemIfNotExists item =
        TransactWrite.Put(item, Some batchDoesNotExistCondition)

    let private updateTip stream updater cond =
        TransactWrite.Update(Batch.tableKeyForStreamTip stream, Some(cce cond), cue updater)
    ...
    let private generateRequests (stream: string) (req, u, exp, b', n') etag' : TransactWrite<Batch.Schema> list =
                    ...
                    <@
                        fun t ->
                            { t with
                                ...
                                u = u
                                n = n'
                                e = Array.append t.e tipE
                                c = Array.append t.c tipC
                            }
                    @>
        ...
        [
            ...
            match exp with
            | Exp.Version 0L
            | Exp.Etag null -> putItemIfNotExists (genFreshTipItem ())
    ...
    let private transactLogged (container, stream) (baseBytes, baseEvents, req, unfolds, exp, b', n') (log: ILogger) : Async<Res> = async {
        let! t, ({ total = ru } as rc, result) = transact (container, stream) (req, unfolds, exp, b', n') |> Stopwatch.Time

        let calfBytes, calfCount, tipBytes, tipEvents, appended =
            req
            |> function
                | Req.Append(_tipWasEmpty, appends) -> 0, 0, baseBytes + Event.arrayBytes appends, baseEvents + appends.Length, appends
                | Req.Calve(calf, tip, appendedCount) ->
                    Event.arrayBytes calf,
                    calf.Length,
                    baseBytes + Event.arrayBytes tip,
                    tip.Length,
        ...
        let outcome, log =
            let reqMetric =
                Log.metric container.TableName stream t (calfBytes + tipBytes) (appended.Length + unfolds.Length) rc
            ...
            | Res.Written etag' ->
                ...
                |> Log.event (
                    (if calfBytes = 0 then
                         Log.Metric.SyncAppend
                     else
                         Log.Metric.SyncCalve)
                        reqMetric
                )
                |> Log.prop "nextPos" n'
            ...
            | Res.ConflictUnknown ->
                ...
                |> Log.event (
                    (if calfBytes = 0 then
                         Log.Metric.SyncAppendConflict
                     else
                         Log.Metric.SyncCalveConflict)
                        reqMetric
                )
                |> Log.prop "conflict" true
                |> Log.prop "eventTypes" (Seq.truncate 5 (seq { for x in appended -> x.c }))

        let appendedBytes, unfoldsBytes =
            Event.arrayBytes appended, Unfold.arrayBytes unfolds

        if calfBytes <> 0 then
            log.Information(
                "EqxDynamo {action:l}{act:l} {outcome:l} {stream:l} {ms:f1}ms {ru}RU {exp:l} {appendedE}e {appendedB}b Tip {baseE}->{tipE}e {baseB}->{tipB}b Unfolds {unfolds} {unfoldsBytes}b Calf {calfEvents} {calfBytes}b",
                "Sync",
                "Calve",
                outcome,
                stream,
            ...
            log.Information(
                "EqxDynamo {action:l}{act:l} {outcome:l} {stream:l} {ms:f1}ms {ru}RU {exp:l} {appendedE}e {appendedB}b Events {events} {tipB}b Unfolds {unfolds} {unfoldsB}b",
                "Sync",
                "Append",
                outcome,
                stream,
    ...
    let handle log (maxEvents, maxBytes) (container, stream) (pos, exp, n', events: IEventData<EncodedBody> array, unfolds: IEventData<EncodedBody> array) = async {
        ...
        let events: Event array =
            ...
            |> Array.mapi (fun i e -> {
                ...
                t = e.Timestamp
                c = e.EventType
                d = EncodedBody.toInternal e.Data
                m = EncodedBody.toInternal e.Meta
                correlationId = Option.ofObj e.CorrelationId
                causationId = Option.ofObj e.CausationId
            })

        let unfolds: Unfold array =
            unfolds
            |> Array.map (fun (x: IEventData<_>) -> {
                i = n'
                t = x.Timestamp
                c = x.EventType
                d = EncodedBody.toInternal x.Data
                m = EncodedBody.toInternal x.Meta
            })

        if Array.isEmpty events && Array.isEmpty unfolds then
            invalidOp "Must write either events or unfolds."

        let cur = Position.flatten pos

        let req, predecessorBytes', tipEvents' =
            let eventOverflow =
                maxEvents |> Option.exists (fun limit -> events.Length + cur.events.Length > limit)

            if eventOverflow || cur.baseBytes + Unfold.arrayBytes unfolds + Event.arrayBytes events > maxBytes then
                let calfEvents, residualEvents =
                    ResizeArray(cur.events.Length + events.Length), ResizeArray()
                ...
                for e in Seq.append cur.events events do
                    ...
                    | false, calfSize' when calfSize' < maxDynamoDbItemSize ->
                        calfSize <- calfSize'
                        calfEvents.Add e
                    | _ ->
                        calfFull <- true
                        residualEvents.Add e
                ...
                Req.Append(Array.isEmpty cur.events, events), cur.calvedBytes, Array.append cur.events events
...
module internal Tip =
    ...
    let private get (container: Container, stream: string) consistentRead (maybePos: Position option) = async {
        match! container.TryGetTip(stream, consistentRead) with
    ...
    let private loggedGet
        (get: Container * string -> bool -> Position option -> Async<_>)
        (container, stream)
        consistentRead
        (maybePos: Position option)
        (log: ILogger)
        =
        async {
            let log = log |> Log.prop "stream" stream
            let! t, ({ total = ru } as rc, res: Res<_>) = get (container, stream) consistentRead maybePos |> Stopwatch.Time

            let logMetric bytes count (f: Log.Measurement -> _) =
                log |> Log.event (f (Log.metric container.TableName stream t bytes count rc))

            match res with
            | Res.NotModified ->
                (logMetric 0 0 Log.Metric.TipNotModified)
                    .Information("EqxDynamo {action:l} {res} {stream:l} {ms:f1}ms {ru}RU", "Tip", 304, stream, t.ElapsedMilliseconds, ru)
            | Res.NotFound ->
                (logMetric 0 0 Log.Metric.TipNotFound)
                    .Information("EqxDynamo {action:l} {res} {stream:l} {ms:f1}ms {ru}RU", "Tip", 404, stream, t.ElapsedMilliseconds, ru)
            | Res.Found tip ->
                let eventsCount, unfoldsCount, bb, ub =
                    tip.e.Length, tip.u.Length, Batch.bytesBase tip, Batch.bytesUnfolds tip

                let log = logMetric (bb + ub) (eventsCount + unfoldsCount) Log.Metric.Tip

                let log =
                    match maybePos with
                    | Some p -> log |> Log.prop "startPos" p |> Log.prop "startEtag" p
                    | None -> log

                log.Information(
                    "EqxDynamo {action:l} {res} {stream:l} v{n} {ms:f1}ms {ru}RU {etag} {events}e {unfolds}u {baseBytes}+{unfoldsBytes}b",
                    "Tip",
                    200,
                    stream,
                    tip.n,
    ...
    let tryLoad
        ...
        containerStream
        consistentRead
        (maybePos: Position option, maxIndex)
        : Async<Res<Position * int64 * ITimelineEvent<InternalBody> array>> =
        async {
            let! _rc, res = loggedGet get containerStream consistentRead maybePos log
...
module internal Query =
    ...
    let private mkQuery (log: ILogger) (container: Container, stream: string) consistentRead maxItems (direction: Direction, minIndex, maxIndex) =
        ...
        container.QueryBatches(stream, consistentRead, minN, maxI, (direction = Direction.Backward), maxItems)
    ...
    let private mapPage
        direction
        (container: Container, stream: string)
        (minIndex, maxIndex, maxItems)
        (maxRequests: int option)
        (log: ILogger)
        (i, t, batches: Batch array, rc)
        : Event array * Position option * RequestConsumption =
        match maxRequests with
        | Some mr when i >= mr ->
            log.Information("EqxDynamo {action:l} Page {page} Limit exceeded", i)
            invalidOp "batch Limit exceeded"
        | _ -> ()

        let unwrapBatch (x: Batch) =
            Batch.enumEvents (minIndex, maxIndex) x |> if direction = Direction.Backward then Seq.rev else id

        let events = batches |> Seq.collect unwrapBatch |> Array.ofSeq

        let usedEventsCount, usedBytes, totalBytes =
            events.Length, Event.arrayBytes events, Batch.bytesTotal batches

        let baseIndex =
            if usedEventsCount = 0 then
                Nullable()
            else
                Nullable(Seq.map Batch.baseIndex batches |> Seq.min)

        let minI, maxI =
            match events with
            | [||] -> Nullable(), Nullable()
            | xs -> Nullable events[0].i, Nullable events[xs.Length - 1].i

        (log
         |> Log.event (Log.Metric.QueryResponse(direction, Log.metric container.TableName stream t totalBytes usedEventsCount rc)))
            .Information(
                "EqxDynamo {action:l} {page} {minIndex}-{maxIndex} {ms:f1}ms {ru}RU {batches}/{batchSize}@{index} {count}e {bytes}/{totalBytes}b {direction:l}",
                "Page",
                i,
                minI,
                maxI,
    ...
    let private logQuery
        (direction, minIndex, maxIndex)
        (container: Container, stream)
        interval
        (responsesCount, events: Event array)
        n
        (rc: RequestConsumption)
        (log: ILogger)
        =
        ...
        let action =
            ...
            | Direction.Forward -> "QueryF"
            | Direction.Backward -> "QueryB"
        ...
        (log |> Log.event evt)
            .Information(
                ...
                responsesCount,
                bytes,
                Option.toNullable minIndex,
                Option.toNullable maxIndex
            )
    ...
    type ScanResult<'event> = {
        ...
        minIndex: int64
        next: int64
        maybeTipPos: Position option
        events: 'event array
    }

    let scanTip
        (tryDecode: ITimelineEvent<EncodedBody> -> 'event voption, isOrigin: 'event -> bool)
        (pos: Position, i: int64, xs: ITimelineEvent<InternalBody> array)
        : ScanResult<'event> =
    ...
    let scan<'event>
        ...
        async {
            ...
            let mergeBatches (log: ILogger) (batchesBackward: AsyncSeq<Event array * Position option * RequestConsumption>) = async {
                ...
                let! events =
                    ...
                    |> AsyncSeq.map (fun (events, maybePos, rc) ->
                        if Option.isNone maybeTipPos then
                            maybeTipPos <- maybePos

                        lastResponse <- Some events
                        ru <- ru + rc.total
            ...
            let batches: AsyncSeq<Event array * Position option * RequestConsumption> =
                mkQuery readLog (container, stream) consistentRead maxItems (direction, minIndex, maxIndex)
            ...
            let decoded =
                if direction = Direction.Forward then
                    Array.chooseV ValueTuple.snd events
                else
                    Seq.chooseV ValueTuple.snd events |> Seq.rev |> Array.ofSeq
                    ...
                    Some
                        {
                            ...
                            minIndex = i
                            next = m + 1L
                            maybeTipPos = maybeTipPos
                            events = decoded
                        }
            | None, Some { index = tipI } ->
                return
                    Some
                        {
                            found = found
    ...
    let walkLazy<'event>
        ...
        (direction, minIndex, maxIndex)
        : AsyncSeq<'event array> =
        asyncSeq {
            let query =
                mkQuery log (container, stream) (*consistentRead*) false maxItems (direction, minIndex, maxIndex)
            ...
            try
                ...
                while ok do
                    ...
                    match maxRequests with
                    | Some mr when i + 1 >= mr ->
                        batchLog.Information "batch Limit exceeded"
                        invalidOp "batch Limit exceeded"
                    | _ -> ()
                    ...
                    | Some(events, _pos, rc) ->
                        ...
                        for x in events do
                            ...
                            | Some e when isOrigin e ->
                                ...
                                log.Information(
                                    "EqxDynamo Stop stream={stream} at={index} {case} used={used} residual={residual}",
                                    stream,
                                    x.i,
                                    x.c,
                                    used,
    ...
    let load
        ...
        : Async<Position option * 'event array> =
        async {
            ...
            | _ ->
                ...
                let events, pos =
                    ...
                    | Some primary ->
                        ...
                        match pos |> Option.orElse primary.maybeTipPos with
                        | Some p -> Pos p
                        | None -> Next primary.next
                    ...
                    if log.IsEnabled Events.LogEventLevel.Debug then
                        (log
                         |> fun log ->
                             match minIndex with
                             | None -> log
                             | Some mi -> log |> Log.prop "minIndex" mi
                         |> fun log ->
                             match maxIndex with
                             | None -> log
                             | Some mi -> log |> Log.prop "maxIndex" mi)
                            .Debug(message)
                ...
                | _, Choice1Of2 allowMissing ->
                    ...
                    if allowMissing then
                        ...
                        return failwithf "Origin event not found; no Archive Table supplied"
                | _, Choice2Of2 fallback ->

                    let maxIndex =
                        match primary with
                        | Some p -> Some p.minIndex
                        | None -> maxIndex // if no batches in primary, high water mark from tip is max
    ...
    let until (log: ILogger) (container: Container, stream: string) maxItems indexInclusive : Async<int * int * int64> = async {
        ...
        let trimTip expectedN count = async {
            ...
            | Some tip, tipRc ->
                ...
                let condExpr: Quotations.Expr<Batch.Schema -> bool> =
                    <@ fun t -> t.etag = Some tip.etag @>
        ...
        let mapPage (i, t: StopwatchInterval, batches: BatchIndices array, rc) =
            ...
            let log =
                let evt = Log.Metric.PruneResponse reqMetric in log |> Log.prop "batchIndex" i |> Log.event evt

            log.Information(
                "EqxDynamo {action:l} {batches} {ms:f1}ms n={next} {ru}RU",
                "PruneResponse",
                batches.Length,
                t.ElapsedMilliseconds,
                Option.toNullable next,
                rc.total
            )
        ...
        let! pt, outcomes =
            ...
            let handle (batches: BatchIndices array, rc) = async {
                let mutable delCharges, batchesDeleted, trimCharges, batchesTrimmed, eventsDeleted, eventsDeferred =
                    0., 0, 0., 0, 0, 0
        ...
        let mutable queryCharges, delCharges, trimCharges, responses, batches, eventsDeleted, eventsDeferred =
            0., 0., 0., 0, 0, 0, 0
        ...
        let reqMetric =
            Log.metric container.TableName stream pt eventsDeleted batches { total = queryCharges }
        ...
        log.Information(
            "EqxDynamo {action:l} {events}/{batches} lwm={lwm} {ms:f1}ms queryRu={queryRu} deleteRu={deleteRu} trimRu={trimRu}",
            "Prune",
            eventsDeleted,
            batches,
            lwm,
...
[<NoComparison; NoEquality>]
type Token = { pos: Position option }
...
module Token =
    ...
    let (|Unpack|) (token: StreamToken) : Position option =
        let t = unbox<Token> token.value in t.pos
...
module Internal =
    ...
    type InternalSyncResult =
        | Written of StreamToken
        | ConflictUnknown

    [<RequireQualifiedAccess; NoComparison; NoEquality>]
    type LoadFromTokenResult<'event> =
        | Unchanged
        | Found of StreamToken * 'event array
...
type QueryOptions([<O; D null>] ?maxItems: int, [<O; D null>] ?getMaxItems: unit -> int, [<O; D null>] ?maxRequests, [<O; D null>] ?ignoreMissingEvents)
...
// Inhibit throwing when events are missing, but no fallback Table has been supplied. Default: false.
 =
    let getMaxItems =
        defaultArg getMaxItems (fun () -> defaultArg maxItems defaultMaxItems)
...
type TipOptions([<O; D null>] ?maxBytes, [<O; D null>] ?maxEvents)
...
type internal StoreClient(container: Container, fallback: Container option, query: QueryOptions, tip: TipOptions) =
    ...
    let loadTip log stream consistentRead pos =
        Tip.tryLoad log (container, stream) consistentRead (pos, None)

    // Always yields events forward, regardless of direction
    member _.Read(log, stream, consistentRead, direction, (tryDecode, isOrigin), ?minIndex, ?maxIndex, ?tip) : Async<StreamToken * 'event array> = async {
        ...
        let walk log container =
            Query.scan log (container, stream) consistentRead query.MaxItems query.MaxRequests direction (tryDecode, isOrigin)
    ...
    member store.Load(log, (stream, maybePos), consistentRead, (tryDecode, isOrigin), checkUnfolds: bool) : Async<StreamToken * 'event array> =
        if not checkUnfolds then
            store.Read(log, stream, consistentRead, Direction.Backward, (tryDecode, isOrigin))
        else
            async {
                match! loadTip log stream consistentRead maybePos with
                | Tip.Res.NotFound -> return Token.empty, Array.empty
                | Tip.Res.NotModified -> return invalidOp "Not applicable"
                | Tip.Res.Found(pos, i, xs) -> return! store.Read(log, stream, consistentRead, Direction.Backward, (tryDecode, isOrigin), tip = (pos, i, xs))
            }

    member _.GetPosition(log, stream, ?pos) : Async<StreamToken> = async {
        match! loadTip log stream (*consistentRead*) false pos with
    ...
    member store.Reload(log, (stream, maybePos: Position option), consistentRead, (tryDecode, isOrigin), ?preview) : Async<LoadFromTokenResult<'event>> =
        let read tipContent = async {
            let! res =
                store.Read(log, stream, consistentRead, Direction.Backward, (tryDecode, isOrigin), minIndex = Position.toIndex maybePos, tip = tipContent)
...
type internal Category<'event, 'state, 'context>(store: StoreClient, codec: IEventCodec<'event, EncodedBody, 'context>) =
    member _.Load(log, stream, requireLeader, initial, checkUnfolds, fold, isOrigin) : Task<struct (StreamToken * 'state)> = task {
        let! token, events = store.Load(log, (stream, None), requireLeader, (codec.TryDecode, isOrigin), checkUnfolds)
    ...
    member _.Reload
        ...
        task {
            match!
                store.Reload(log, (stream, pos), requireLeader, (codec.TryDecode, isOrigin), ?preview = preloaded)
...
module internal Caching =
    ...
    let applyCacheUpdatesWithSlidingExpiration (cache: ICache, prefix: string) (slidingExpiration: TimeSpan) =
        let mkCacheEntry (initialToken: StreamToken, initialState: 'state) =
            CacheEntry<'state>(initialToken, initialState)

        let options = CacheItemOptions.RelativeExpiration slidingExpiration
        fun streamName value -> cache.UpdateIfNewer(prefix + streamName, options, Token.supersedes, mkCacheEntry value)
    ...
    let applyCacheUpdatesWithFixedTimeSpan (cache: ICache, prefix: string) (period: TimeSpan) =
        ...
        fun streamName value ->
            let expirationPoint =
                let creationDate = DateTimeOffset.UtcNow in creationDate.Add period
    ...
    type CachingCategory<'event, 'state, 'context>
        (
            ...
            isOrigin: 'event -> bool,
            tryReadCache,
            updateCache: string -> _ -> Task<unit>,
            checkUnfolds,
        ...
        interface ICategory<'event, 'state, 'context> with
            member _.Load(log, _categoryName, _streamId, streamName, allowStale, requireLeader, ct) : Task<struct (StreamToken * 'state)> = task {
                ...
                | ValueSome struct (token, state) ->
                    return! (fun () -> category.Reload(log, streamName, requireLeader, token, state, fold, isOrigin, ct)) |> cache streamName
            }
...
[<RequireQualifiedAccess>]
type ConnectionMode =
    | AwsEnvironment of systemName: string
    | AwsKeyCredentials of serviceUrl: string
...
type DynamoStoreConnector(clientConfig: Amazon.DynamoDBv2.AmazonDynamoDBConfig, ?credentials: Amazon.Runtime.AWSCredentials) =
    ...
    new(serviceUrl, accessKey, secretKey, timeout: TimeSpan, retries) =
        ...
        let clientConfig =
            Amazon.DynamoDBv2.AmazonDynamoDBConfig(ServiceURL = serviceUrl, RetryMode = m, MaxErrorRetry = retries, Timeout = timeout)
    ...
    new(systemName, timeout: TimeSpan, retries) =
        ...
        let clientConfig =
            Amazon.DynamoDBv2.AmazonDynamoDBConfig(RegionEndpoint = regionEndpoint, RetryMode = m, MaxErrorRetry = retries, Timeout = timeout)
...
// Facilitates custom mapping of Stream Category Name to underlying Table and Stream names
 =
    ...
    new(client: Amazon.DynamoDBv2.IAmazonDynamoDB,
        tableName: string,
    ...
    // Client to use for archive store. Default: (if <c>archiveTableName</c> specified) use same <c>archiveTableName</c> but via <c>client</c>.
     =
        ...
        let catAndStreamToTableStream (categoryName, streamId) =
            tableName, genStreamName (categoryName, streamId)

        let primaryContainer t = Container.Create(client, t)

        let fallbackContainer =
            if Option.isNone archiveClient && Option.isNone archiveTableName then
                fun _ -> None
            else
                fun primaryContainerName -> Some(Container.Create(defaultArg archiveClient client, defaultArg archiveTableName primaryContainerName))
    ...
    member internal _.ResolveContainerFallbackAndStreamName(categoryName, streamId) : Container * Container option * string =
        let tableName, streamName =
            categoryAndStreamIdToTableAndStreamNames (categoryName, streamId)
    ...
    static member Connect(client, tableName: string, [<O; D null>] ?archiveTableName, [<O; D null>] ?mode: ConnectMode) : Async<DynamoStoreClient> = async {
        let init t =
            ConnectMode.apply client t (defaultArg mode ConnectMode.Verify)

        do! init tableName

        match archiveTableName with
        | None -> ()
        | Some archiveTable -> do! init archiveTable
...
type DynamoStoreContext(storeClient: DynamoStoreClient, tipOptions, queryOptions) =
    ...
    // Inhibit throwing when events are missing, but no Archive Table has been supplied as a fallback
     =
        ...
        let queryOptions =
            QueryOptions(?maxItems = queryMaxItems, ?maxRequests = queryMaxRequests, ?ignoreMissingEvents = ignoreMissingEvents)
    ...
    member internal x.ResolveContainerClientAndStreamId(categoryName, streamId) =
        let container, fallback, streamName =
            storeClient.ResolveContainerFallbackAndStreamName(categoryName, streamId)
...
type DynamoStoreCategory<'event, 'state, 'context>(resolveInner, empty) =
    ...
    new(context: DynamoStoreContext, codec, fold, initial, caching, access) =
        let categories =
            System.Collections.Concurrent.ConcurrentDictionary<string, ICategory<'event, 'state, 'context>>()
        ...
        let resolveCategory (categoryName, container) =
            let createCategory _name : ICategory<_, _, 'context> =
                ...
                let isOrigin, checkUnfolds, mapUnfolds =
                    ...
                    | AccessStrategy.RollingState toSnapshot ->
                        (fun _ -> true), true, Choice3Of3(fun _ state -> Array.empty, toSnapshot state |> Array.singleton)
        ...
        let resolveInner struct (categoryName, streamId) =
            let struct (container, streamName) =
                context.ResolveContainerClientAndStreamId(categoryName, streamId)
...
module Exceptions =
    ...
    [<return: Struct>]
    let (|ProvisionedThroughputExceeded|_|): exn -> unit voption =
        function
...
type EventsContext internal (context: Equinox.DynamoStore.DynamoStoreContext, store: StoreClient, log: Serilog.ILogger)
// Logger to write to - see https://github.com/serilog/serilog/wiki/Provided-Sinks for how to wire to your logger
 =
    ...
    new(context: Equinox.DynamoStore.DynamoStoreContext, log) =
        let storeClient =
            context.ResolveContainerClientAndStreamId(null, null) |> ValueTuple.fst

        EventsContext(context, storeClient, log)

    member x.StreamId(streamName) : string =
        context.ResolveContainerClientAndStreamId(null, streamName) |> ValueTuple.snd
    ...
    member internal _.GetLazy(stream, ?queryMaxItems, ?direction, ?minIndex, ?maxIndex) : AsyncSeq<ITimelineEvent<EncodedBody> array> =
        ...
        let batching =
            match queryMaxItems with
            | Some qmi -> QueryOptions(qmi)
            | _ -> context.QueryOptions
    ...
    member internal _.GetInternal(stream, ?minIndex, ?maxIndex, ?maxCount, ?direction) = async {
        ...
        if maxCount = Some 0 then
            ...
            let! token, events =
                store.Read(log, stream (*consistentRead*) , false, direction, (ValueSome, isOrigin), ?minIndex = minIndex, ?maxIndex = maxIndex)
    ...
    member x.Read
        ...
        x.GetInternal(stream, ?minIndex = minIndex, ?maxIndex = maxIndex, ?maxCount = maxCount, ?direction = direction)
        |> yieldPositionAndData
#if APPEND_SUPPORT
    ...
    member _.Prune(stream, index) : Async<int * int * int64> = store.Prune(log, stream, index)
...
type EventData() =
    ...
    static member FromUtf8Bytes(eventType, data, ?meta) : IEventData<_> =
        FsCodec.Core.EventData.Create(eventType, data, ?meta = meta) :> _
...
module Events =
    ...
    let get (ctx: EventsContext) (streamName: string) (index: int64) (maxCount: int) : Async<ITimelineEvent<EncodedBody> array> =
        ctx.Read(ctx.StreamId streamName, ?minIndex = (if index = 0 then None else Some index), maxCount = maxCount)
        |> dropPosition
