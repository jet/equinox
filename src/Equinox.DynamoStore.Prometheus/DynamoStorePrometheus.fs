...
module private Histograms =
    ...
    let labelNames tagNames =
        Array.append tagNames [| "rut"; "facet"; "op"; "table"; "cat" |]

    let labelValues tagValues (rut, facet, op, table, cat) =
        Array.append tagValues [| rut; facet; op; table; cat |]
    ...
    let private mkHistogram (cfg: Prometheus.HistogramConfiguration) name desc =
        ...
        fun tagValues (rut, facet: string, op: string) (table, cat: string) s -> h.WithLabels(labelValues tagValues (rut, facet, op, table, cat)).Observe(s)
    ...
    let private sHistogram tagNames =
        ...
        let sCfg =
            Prometheus.HistogramConfiguration(Buckets = sBuckets, LabelNames = labelNames tagNames)
    ...
    let private ruHistogram tagNames =
        ...
        let ruCfg =
            Prometheus.HistogramConfiguration(Buckets = ruBuckets, LabelNames = labelNames tagNames)
...
module private Summaries =
    ...
    let labelNames tagNames =
        Array.append tagNames [| "facet"; "table" |]

    let labelValues tagValues (facet, table) =
        Array.append tagValues [| facet; table |]
    ...
    let sAndRuPair (tagNames, tagValues) stat desc =
        ...
        let observeS =
            mkSummary (config tagNames) (baseName + "_seconds") (baseDesc + " latency") tagValues

        let observeRu =
            mkSummary (config tagNames) (baseName + "_ru") (baseDesc + " charge") tagValues
...
module private Counters =
    ...
    let labelNames tagNames =
        Array.append tagNames [| "facet"; "op"; "outcome"; "table"; "cat" |]

    let labelValues tagValues (facet, op, outcome, table, cat) =
        Array.append tagValues [| facet; op; outcome; table; cat |]

    let private mkCounter (cfg: Prometheus.CounterConfiguration) name desc =
        let h = Prometheus.Metrics.CreateCounter(name, desc, cfg)
        fun tagValues (facet: string, op: string, outcome: string) (table, cat) c -> h.WithLabels(labelValues tagValues (facet, op, outcome, table, cat)).Inc(c)

    let config tagNames =
        Prometheus.CounterConfiguration(LabelNames = labelNames tagNames)
    ...
    let eventsAndBytesPair tags stat desc =
        ...
        fun ctx (table, cat, e, b) ->
            ...
            match b with
            | None -> ()
            | Some b -> observeB ctx (table, cat) b
...
type LogSink(customTags: seq<string * string>) =
    ...
    let roundtripSummary =
        Summaries.sAndRuPair tags "roundtrip_summary" "Fragment Summary"
