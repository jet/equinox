...
type EsSyncResult =
    | Written of ConditionalWriteResult
    | Conflict of actualVersion: int64

module private Write =

    let private writeEventsAsync (log: ILogger) (conn: EventStoreClient) (streamName: string) version (events: EventData[]) : Async<EsSyncResult> = async {
        let! ct = Async.CancellationToken

        let! wr =
            conn.ConditionalAppendToStreamAsync(streamName, StreamRevision.FromInt64 version, events, cancellationToken = ct)
            |> Async.AwaitTaskCorrect

        if wr.Status = ConditionalWriteStatus.VersionMismatch then
            log.Information(
                "Esdb TrySync VersionMismatch writing {EventTypes}, actual {ActualVersion}",
                [| for x in events -> x.Type |],
                wr.NextExpectedVersion
            )

            return EsSyncResult.Conflict wr.NextExpectedVersion
        elif wr.Status = ConditionalWriteStatus.StreamDeleted then
            return failwithf "Unexpected write to deleted stream %s" streamName
        elif wr.Status = ConditionalWriteStatus.Succeeded then
            return EsSyncResult.Written wr
        else
            return failwithf "Unexpected write response code %O" wr.Status
    }
    ...
    let private writeEventsLogged (conn: EventStoreClient) (streamName: string) (version: int64) (events: EventData[]) (log: ILogger) : Async<EsSyncResult> = async {
        let log =
        ...
        (resultLog |> Log.event evt)
            .Information(
                "Esdb{action:l} count={count} conflict={conflict}",
                "Write",
                events.Length,
                match evt with
    ...
    let writeEvents (log: ILogger) retryPolicy (conn: EventStoreClient) (streamName: string) (version: int64) (events: EventData[]) : Async<EsSyncResult> =
...
module private Read =
    ...
    let resolvedEventBytes (x: ResolvedEvent) =
        let Log.BlobLen bytes, Log.BlobLen metaBytes = x.Event.Data, x.Event.Metadata in bytes + metaBytes

    let resolvedEventsBytes events =
        events |> Array.sumBy resolvedEventBytes
    ...
    let logBatchRead direction streamName t events batchSize version (log: ILogger) =
        ...
        let batches =
            ...
            | Some batchSize -> (events.Length - 1) / batchSize + 1
            | None -> -1
        ...
        let log =
            if (not << log.IsEnabled) Events.LogEventLevel.Debug then
                ...
                log |> Log.propResolvedEvents "Json" events

        let evt = Log.Metric.Batch(direction, batches, reqMetric)

        (log |> Log.prop "bytes" bytes |> Log.event evt)
            .Information("Esdb{action:l} stream={stream} count={count}/{batches} version={version}", action, streamName, count, batches, version)

    let loadBackwardsUntilOrigin
        (log: ILogger)
        (conn: EventStoreClient)
        batchSize
        streamName
        (tryDecode, isOrigin)
        ...
        async {
            ...
            let res =
                conn.ReadStreamAsync(Direction.Backwards, streamName, StreamPosition.End, int64 batchSize, resolveLinkTos = false, cancellationToken = ct)
    ...
    let loadForward (conn: EventStoreClient) streamName startPosition : Async<int64 * ResolvedEvent[]> = async {
        let! ct = Async.CancellationToken

        let res =
            conn.ReadStreamAsync(Direction.Forwards, streamName, startPosition, Int64.MaxValue, resolveLinkTos = false, cancellationToken = ct)

        try
            let! events = AsyncSeq.ofAsyncEnum res |> AsyncSeq.toArrayAsync

            let v =
                match Seq.tryLast events with
                | Some r -> let en = r.Event.EventNumber in en.ToInt64()
                | None -> startPosition.ToInt64() - 1L
    ...
    let loadForwards log conn streamName startPosition : Async<int64 * ResolvedEvent[]> = async {
        let direction = Direction.Forward
        let! t, (version, events) = loadForward conn streamName startPosition |> Stopwatch.Time

        let log =
            log |> Log.prop "startPos" startPosition |> Log.prop "direction" direction |> Log.prop "stream" streamName
...
type EventStoreContext(connection: EventStoreConnection, batchOptions: BatchOptions) =
    ...
    member internal _.LoadBackwardsStoppingAtCompactionEvent
        (
            streamName,
            requireLeader,
            log,
            ...
            tryDecode,
            isOrigin
        ) : Async<struct (StreamToken * 'event[])> =
        async {
    ...
    member internal _.TrySync(log, streamName, streamToken, events, encodedEvents: EventData array, isCompactionEventType) : Async<GatewaySyncResult> = async {
...
type private Category<'event, 'state, 'context>(context: EventStoreContext, codec: FsCodec.IEventCodec<_, _, 'context>, ?access: AccessStrategy<'event, 'state>) =
    ...
    let tryDecode (e: ResolvedEvent) =
        e.Event |> ClientCodec.timelineEvent |> codec.TryDecode
    ...
    let loadAlgorithm streamName requireLeader log =
        let compacted limit =
            context.LoadBackwardsStoppingAtCompactionEvent(streamName, requireLeader, log, limit, tryDecode, isOrigin)
    ...
    member x.TrySync<'context>
        ...
        async {
            ...
            let encodedEvents: EventData[] =
                events |> Array.map (encode >> ClientCodec.eventData)
            ...
            | GatewaySyncResult.ConflictUnknown _ ->
                return SyncResult.Conflict(fun ct -> x.Reload(fold, state, streamName (*requireLeader*) , true, streamToken, log) |> Async.startAsTask ct)
...
type EventStoreConnector
    ...
    member _.Connect
        ...
        match clusterNodePreference with
        | None -> ()
        | Some np -> settings.ConnectivitySettings.NodePreference <- np

        match customize with
        | None -> ()
        | Some f -> f settings
