...
let applyCacheUpdatesWithSlidingExpiration
    ...
    let mkCacheEntry struct (initialToken: StreamToken, initialState: 'state) =
        new CacheEntry<'state>(initialToken, initialState)

    let options = CacheItemOptions.RelativeExpiration slidingExpiration

    let addOrUpdateSlidingExpirationCacheEntry streamName value =
...
let applyCacheUpdatesWithFixedTimeSpan
    ...
    let addOrUpdateFixedLifetimeCacheEntry streamName value =
        let expirationPoint =
            let creationDate = System.DateTimeOffset.UtcNow in creationDate.Add lifetime
