...
type StreamEventsSlice = {
    Messages: ITimelineEvent<EventBody> array
    IsEnd: bool
    LastVersion: int64
}
...
module Log =
    ...
    let bytesToString (bytes: EventBody) =
    ...
    module InternalMetrics =
        ...
        module Stats =
            ...
            let (|Read|ReadL|Write|Resync|Rollup|) =
                function
...
module private Write =
    let private writeEventsAsync
        (log: ILogger)
        (writer: MessageDbWriter)
        (streamName: string)
        (version: int64)
        (events: IEventData<EventBody> array)
        : Async<MdbSyncResult> =
        async {
            ...
            | MdbSyncResult.ConflictUnknown ->
                log.Information(
                    "MdbTrySync WrongExpectedVersionException writing {EventTypes}, expected {ExpectedVersion}",
                    [| for x in events -> x.EventType |],
                    version
                )
    ...
    let private writeEventsLogged
        ...
        async {
            ...
            let resultLog, evt =
                match result with
                | MdbSyncResult.ConflictUnknown -> log, Log.WriteConflict reqMetric
                | MdbSyncResult.Written x -> log |> Log.prop "currentPosition" x, Log.WriteSuccess reqMetric

            (resultLog |> Log.event evt)
                .Information(
                    "Mdb{action:l} count={count} conflict={conflict}",
                    "Write",
                    events.Length,
                    match evt with
...
module private Read =
    ...
    let private readSliceAsync (reader: MessageDbReader) (streamName: string) (batchSize: int64) (startPos: int64) (requiresLeader: bool) ct : Task<_> = task {
    ...
    let private loggedReadSlice reader streamName batchSize startPos requiresLeader (log: ILogger) : Async<_> = async {
        ...
        (log |> Log.prop "startPos" startPos |> Log.prop "bytes" bytes |> Log.event evt)
            .Information("Mdb{action:l} count={count} version={version}", "Read", count, slice.LastVersion)
    ...
    let logBatchRead streamName t events (batchSize: int64) version (log: ILogger) =
        ...
        (log |> Log.prop "bytes" bytes |> Log.event evt)
            .Information("Mdb{action:l} stream={stream} count={count}/{batches} version={version}", action, streamName, count, batches, version)
    ...
    let logLastEventRead streamName t events version (log: ILogger) =
        ...
        (log |> Log.prop "bytes" bytes |> Log.event evt)
            .Information("Mdb{action:l} stream={stream} count={count} version={version}", "ReadL", streamName, count, version)

    let loadLastEvent (log: ILogger) retryPolicy (reader: MessageDbReader) requiresLeader streamName : Async<int64 * ITimelineEvent<EventBody> array> = async {
        let! ct = Async.CancellationToken

        let read _ =
            readLastEventAsync reader streamName requiresLeader ct |> Async.AwaitTaskCorrect
    ...
    let loadForwardsFrom
        ...
        startPosition
        requiresLeader
        : Async<int64 * ITimelineEvent<EventBody> array> =
        async {
            let mergeBatches (batches: AsyncSeq<int64 * ITimelineEvent<EventBody> array>) = async {
                ...
                let! (events: ITimelineEvent<EventBody> array) =
                    batches
                    |> AsyncSeq.map (fun (reportedVersion, events) ->
                        versionFromStream <- max reportedVersion versionFromStream
                        events)
                    |> AsyncSeq.concatSeq
            ...
            let call pos =
                loggedReadSlice reader streamName batchSize pos requiresLeader
            ...
            let batches: AsyncSeq<int64 * ITimelineEvent<EventBody> array> =
...
module private Token =
    ...
    let supersedes struct (current, x) = x.version > current.version
...
type MessageDbContext(connection: MessageDbConnection, batchOptions: BatchOptions) =
    new(connection: MessageDbConnection, [<O; D null>] ?batchSize)
    ...
    member _.LoadBatched(streamName, requireLeader, log, tryDecode) : Async<StreamToken * 'event array> = async {
        let! version, events =
            Read.loadForwardsFrom log connection.ReadRetryPolicy connection.Reader batchOptions.BatchSize batchOptions.MaxBatches streamName 0L requireLeader
    ...
    member _.Reload(streamName, requireLeader, log, token, tryDecode) : Async<StreamToken * 'event array> = async {
    ...
    member _.TrySync(log, streamName, token, encodedEvents: IEventData<EventBody> array) : Async<GatewaySyncResult> = async {
        ...
        | MdbSyncResult.ConflictUnknown -> return GatewaySyncResult.ConflictUnknown
...
type MessageDbCategory<'event, 'state, 'context>(resolveInner, empty) =
    ...
    new(context: MessageDbContext, codec: IEventCodec<_, _, 'context>, fold, initial, [<O; D(null)>] ?caching, [<O; D(null)>] ?access) =
        ...
        let resolveInner streamIds =
            struct (category, StreamName.Internal.ofCategoryAndStreamId streamIds, ValueNone)
