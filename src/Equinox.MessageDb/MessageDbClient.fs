...
module private Json =
    ...
    let toArray (m: Format) =
        if m.IsEmpty then jsonNull else m.ToArray()
...
type MessageDbWriter(connectionString: string) =
    ...
    member private _.PrepareWriteCommand(streamName: string, version, message: IEventData<Format>) =
        let cmd =
            NpgsqlBatchCommand(CommandText = "select 1 from write_message(@Id::text, @StreamName, @EventType, @Data, @Meta, @ExpectedVersion)")
    ...
    member client.WriteMessages(streamName, events: _ array, version: int64, ct) = task {
        try
            ...
            let addCommand i event =
                client.PrepareWriteCommand(streamName, version + int64 i, event) |> batch.BatchCommands.Add
...
type MessageDbReader internal (connectionString: string, leaderConnectionString: string) =
    ...
    let readRow (reader: DbDataReader) =
        let readNullableString idx =
            if reader.IsDBNull(idx) then
                None
            else
                Some(reader.GetString idx)
