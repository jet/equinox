...
module private Deflate =
    ...
    let compress (uncompressedBytes: byte array) =
        ...
        let compressor =
            new System.IO.Compression.DeflateStream(output, System.IO.Compression.CompressionLevel.Optimal, leaveOpen = true)
...
module JsonElement =
    ...
    let undefinedToNull (e: JsonElement) =
        if e.ValueKind = JsonValueKind.Undefined then
            nullElement
        else
            e
    ...
    let private optionsNoEscaping =
        JsonSerializerOptions(Encoder = System.Text.Encodings.Web.JavaScriptEncoder.UnsafeRelaxedJsonEscaping)

    let private toUtf8Bytes (value: JsonElement) =
        JsonSerializer.SerializeToUtf8Bytes(value, options = optionsNoEscaping)

    let deflate (value: JsonElement) : JsonElement =
        if value.ValueKind = JsonValueKind.Null then
            value
        else
            value |> toUtf8Bytes |> Deflate.compress |> JsonSerializer.SerializeToElement
