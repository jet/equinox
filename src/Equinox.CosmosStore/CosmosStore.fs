...
module internal Position =
    ...
    let tryFromBatch (x: Batch) =
        if x.id <> Tip.WellKnownDocumentId then
            None
        else
            Some
                {
...
module Log =
    ...
    let internal propData name (events: #IEventData<EventBody> seq) (log: ILogger) =
        ...
        let items = seq {
            for e in events do
                yield sprintf "{\"%s\": %s}" e.EventType (render e.Data)
        }
    ...
    let internal (|BlobLen|) (x: EventBody) =
        if x.ValueKind = JsonValueKind.Null then
            0
        else
            x.GetRawText().Length

    let internal eventLen (x: #IEventData<_>) =
        let BlobLen bytes, BlobLen metaBytes = x.Data, x.Meta in bytes + metaBytes + 80
    ...
    module InternalMetrics =
        ...
        module Stats =
            ...
            let inline private (|RcMs|) ({ interval = i; ru = ru }: Measurement) = ru, int64 i.ElapsedMilliseconds
            ...
            type LogSink() =
                ...
                interface Serilog.Core.ILogEventSink with
                    member _.Emit logEvent =
                        ...
                        | MetricEvent cm ->
                            match cm with
                            | Op((Operation.Tip | Operation.Tip404 | Operation.Tip304 | Operation.Query), RcMs m) -> LogSink.Read.Ingest m
        ...
        let dump (log: ILogger) =
            ...
            let logPeriodicRate name count rru wru =
                log.Information("{rru:n1}R/{wru:n1}W CU @ {count:n0} rp{unit}", rru, wru, count, name)
...
module private MicrosoftAzureCosmosWrappers =
    ...
    type Container with
        ...
        member private container.DeserializeResponseBody<'T>(rm: ResponseMessage) : 'T =
            rm.EnsureSuccessStatusCode().Content |> container.Database.Client.ClientOptions.Serializer.FromStream<'T>
        ...
        member container.TryReadItem(partitionKey: PartitionKey, id: string, ?options: ItemRequestOptions) : Async<float * ReadResult<'T>> = async {
            ...
            use! rm =
                container.ReadItemStreamAsync(id, partitionKey, requestOptions = Option.toObj options, cancellationToken = ct)
                |> Async.AwaitTaskCorrect

            return
                rm.Headers.RequestCharge,
                rm.StatusCode
                |> function
...
module internal SyncStoredProc =
    [<Literal>]
    let name = "EquinoxEventsInTip4" // NB need to rename/number for any breaking change
...
module internal Sync =
    ...
    let private run (container: Container, stream: string) (maxEventsInTip, maxStringifyLen) (exp, req: Tip) : Async<float * Result> = async {
        let ep =
            match exp with
        ...
        let newPos = {
            index = res.Resource.n
            etag = Option.ofObj res.Resource.etag
        }
        ...
        | [||] when res.Resource.e.Length = 0 && newPos.index > ep.index -> return res.RequestCharge, Result.ConflictUnknown newPos
        | unfolds -> // stored proc only returns events and unfolds with index >= req.i - no need to trim to a minIndex
            let events =
                (Enum.Events(ep.index, res.Resource.e), Enum.Unfolds unfolds) ||> Seq.append |> Array.ofSeq

            return res.RequestCharge, Result.Conflict(newPos, events)
    }

    let private logged (container, stream) (maxEventsInTip, maxStringifyLen) (exp: SyncExp, req: Tip) (log: ILogger) : Async<Result> = async {
        ...
        let count, bytes =
            req.e.Length, (if verbose then Enum.Events req |> Log.batchLen else 0)
        ...
        let log =
            ...
            let inline propConflict log =
                log |> Log.prop "conflict" true |> Log.prop "eventTypes" (Seq.truncate 5 (seq { for x in req.e -> x.c }))

            if verbose then
                log |> Log.propEvents (Enum.Events req) |> Log.propDataUnfolds req.u
            else
                log
            |> match exp with
            ...
            |> match result with
               | Result.Written pos -> Log.prop "nextExpectedVersion" pos >> Log.event (Log.Metric.SyncSuccess(mkMetric ru))
               | Result.ConflictUnknown pos' -> Log.prop "nextExpectedVersion" pos' >> propConflict >> Log.event (Log.Metric.SyncConflict(mkMetric ru))
               | Result.Conflict(pos', xs) ->
                   if verbose then Log.propData "conflicts" xs else id
                   >> Log.prop "nextExpectedVersion" pos'
                   >> propConflict
                   >> Log.event (Log.Metric.SyncResync(mkMetric ru))

        log.Information(
            "EqxCosmos {action:l} {stream} {count}+{ucount} {ms:f1}ms {ru}RU {bytes:n0}b {exp}",
            "Sync",
            stream,
            count,
            req.u.Length,
    ...
    let private mkEvent (e: IEventData<_>) = {
        t = e.Timestamp
        c = e.EventType
        d = JsonElement.undefinedToNull e.Data
        m = JsonElement.undefinedToNull e.Meta
    ...
    let mkBatch (stream: string) (events: IEventData<_>[]) unfolds : Tip = {
        p = stream
        id = Tip.WellKnownDocumentId
        n = -1L (*Server-managed*)
        i = -1L (*Server-managed*)
        _etag = null
        e = Array.map mkEvent events
...
module Initialization =
    ...
    type Throughput =
        | Manual of rus: int
        | Autoscale of maxRus: int

    [<RequireQualifiedAccess>]
    type Provisioning =
        | Container of Throughput
        | Database of Throughput
        | Serverless

    let private (|ThroughputProperties|) =
        function
    ...
    let private createOrProvisionDatabase (client: CosmosClient) dName mode = async {
        ...
        let createDatabaseIfNotExists maybeTp = async {
            let! r =
                client.CreateDatabaseIfNotExistsAsync(dName, throughputProperties = Option.toObj maybeTp, cancellationToken = ct)
                |> Async.AwaitTaskCorrect
        ...
        | Provisioning.Serverless -> return! createDatabaseIfNotExists None
    ...
    let private createOrProvisionContainer (d: Database) (cName, pkPath, customizeContainer) mode = async {
        ...
        let createContainerIfNotExists maybeTp = async {
            let! r =
                d.CreateContainerIfNotExistsAsync(cp, throughputProperties = Option.toObj maybeTp, cancellationToken = ct)
                |> Async.AwaitTaskCorrect
        ...
        | Provisioning.Serverless -> return! createContainerIfNotExists None
    ...
    let private createStoredProcIfNotExists (c: Container) (name, body) : Async<float> = async {
        ...
        try
            let! r =
                c.Scripts.CreateStoredProcedureAsync(Scripts.StoredProcedureProperties(id = name, body = body), cancellationToken = ct)
                |> Async.AwaitTaskCorrect

            return r.RequestCharge
        with :? CosmosException as ce when ce.StatusCode = System.Net.HttpStatusCode.Conflict ->
            return ce.RequestCharge
    }
    ...
    let private applyBatchAndTipContainerProperties (cp: ContainerProperties) =
        ...
        for k in Batch.IndexedFields do
            cp.IndexingPolicy.IncludedPaths.Add(IncludedPath(Path = sprintf "/%s/?" k))
    ...
    let initAux (client: CosmosClient) (dName, cName) mode = async {
        let! d = createOrProvisionDatabase client dName mode
        return! createOrProvisionContainer d (cName, "/id", applyAuxContainerProperties) mode
    } // as per Cosmos team, Partition Key must be "/id"
    ...
    type internal ContainerInitializerGuard(container: Container, fallback: Container option, ?initContainer: Container -> CancellationToken -> Task<unit>) =
        let initGuard =
            initContainer |> Option.map (fun init -> AsyncCacheCell<unit>(init container))

        member _.Container = container
        member _.Fallback = fallback

        member internal _.InitializationGate =
            match initGuard with
            | Some g when not (g.IsValid()) -> ValueSome g.Await
            | _ -> ValueNone
...
module internal Tip =
    ...
    let private get (container: Container, stream: string) (maybePos: Position option) =
        let ro =
            match maybePos with
            | Some { etag = Some etag } -> ItemRequestOptions(IfNoneMatchEtag = etag) |> Some
            | _ -> None
    ...
    let private loggedGet (get: Container * string -> Position option -> Async<_>) (container, stream) (maybePos: Position option) (log: ILogger) = async {
        ...
        | ReadResult.NotModified ->
            (log 0 0 Log.Metric.TipNotModified)
                .Information("EqxCosmos {action:l} {stream} {res} {ms:f1}ms {ru}RU", "Tip", stream, 304, t.ElapsedMilliseconds, ru)
        | ReadResult.NotFound ->
            (log 0 0 Log.Metric.TipNotFound)
                .Information("EqxCosmos {action:l} {stream} {res} {ms:f1}ms {ru}RU", "Tip", stream, 404, t.ElapsedMilliseconds, ru)
        | ReadResult.Found tip ->
            let log =
                let count, bytes =
                    tip.u.Length, (if verbose then Enum.Unfolds tip.u |> Log.batchLen else 0)
            ...
            let log =
                match maybePos with
                | Some p -> log |> Log.propStartPos p |> Log.propStartEtag p
                | None -> log
    ...
    type Result =
        | NotModified
        | NotFound
        | Found of Position * i: int64 * ITimelineEvent<EventBody>[]
...
module internal Query =
    ...
    let private mkQuery
        ...
        includeTip
        (maxItems: int)
        (direction: Direction, minIndex, maxIndex)
        : FeedIterator<Batch> =
        ...
        let query =
            let args = [
                match minIndex with
                | None -> ()
                | Some x -> yield "c.n > @minPos", (fun (q: QueryDefinition) -> q.WithParameter("@minPos", x))
                match maxIndex with
                | None -> ()
                | Some x -> yield "c.i < @maxPos", (fun (q: QueryDefinition) -> q.WithParameter("@maxPos", x))
            ]
            ...
            let whereClause =
                ...
                if List.isEmpty args && includeTip then
                    null
                else
                    "WHERE "
                    + String.Join(
                        ...
                        if includeTip then
                            ...
                            Seq.append conditions (Seq.singleton notTip)
                    )

            let queryString =
                sprintf "SELECT c.id, c.i, c._etag, c.n, c.e FROM c %s ORDER BY c.i %s" whereClause order

            let prams = Seq.map snd args
            (QueryDefinition queryString, prams) ||> Seq.fold (fun q wp -> q |> wp)

        log.Debug(
            "EqxCosmos Query {stream} {query}; n>{minIndex} i<{maxIndex}",
            stream,
            query.QueryText,
    ...
    let private mapPage
        ...
        let unwrapBatch (b: Batch) =
            Enum.Events(b, ?minIndex = minIndex, ?maxIndex = maxIndex) |> if direction = Direction.Backward then Seq.rev else id
        ...
        let log =
            let evt = Log.Metric.QueryResponse(direction, reqMetric) in log |> Log.event evt
        ...
        let index =
            if count = 0 then
                ...
                Nullable <| Seq.min (seq { for x in batches -> x.i })

        (log
         |> Log.prop "bytes" bytes
         |> match minIndex with
            | None -> id
            | Some i -> Log.prop "minIndex" i
         |> match maxIndex with
            | None -> id
            | Some i -> Log.prop "maxIndex" i)
            .Information(
                "EqxCosmos {action:l} {count}/{batches} {direction} {ms:f1}ms i={index} {ru}RU",
                "Response",
                count,
                batches.Length,
                direction,
    ...
    let private logQuery
        direction
        (container: Container, streamName)
        interval
        (responsesCount, events: ITimelineEvent<EventBody>[])
        n
        (ru: float)
        (log: ILogger)
        =
        ...
        (log |> Log.prop "bytes" bytes |> Log.event evt)
            .Information(
    ...
    let scanTip
        (tryDecode: #IEventData<EventBody> -> 'event voption, isOrigin: 'event -> bool)
        (pos: Position, i: int64, xs: #ITimelineEvent<EventBody>[])
        : ScanResult<'event> =
                    ...
                    |> AsyncSeq.takeWhileInclusive (function
                        | struct (x, ValueSome e) when isOrigin e ->
                            ...
                            | Some batch ->
                                ...
                                log.Information(
                                    "EqxCosmos Stop stream={stream} at={index} {case} used={used} residual={residual}",
                                    stream,
                                    x.Index,
                                    x.EventType,
                                    used,
    ...
    let walkLazy<'event>
        ...
        asyncSeq {
            let query =
                mkQuery log (container, stream) true maxItems (direction, minIndex, maxIndex)

            let readPage =
                mapPage direction (container, stream) (minIndex, maxIndex) maxRequests
            ...
            | _ ->
                ...
                | _, Choice1Of2 allowMissing ->
                    ...
                    if allowMissing then
                        ...
                        return failwithf "Origin event not found; no Archive Container supplied"
...
module Prune =
    ...
    let until (log: ILogger) (container: Container, stream: string) (maxItems: int) indexInclusive : Async<int * int * int64> = async {
        ...
        let trimTip expectedI count = async {
            ...
            | tipRu, ReadResult.Found tip ->
                ...
                let tip =
                    { tip with
                        i = tip.i + int64 count
                        e = Array.skip count tip.e
                    }

                let ro =
                    ItemRequestOptions(EnableContentResponseOnWrite = false, IfMatchEtag = tip._etag)
        ...
        let query: FeedIterator<BatchIndices> =
            let qro =
                QueryRequestOptions(PartitionKey = PartitionKey stream, MaxItemCount = maxItems)
        ...
        log.Information(
            "EqxCosmos {action:l} {events}/{batches} lwm={lwm} {ms:f1}ms queryRu={queryRu} deleteRu={deleteRu} trimRu={trimRu}",
            "Prune",
            eventsDeleted,
            batches,
            lwm,
...
module Token =
    ...
    let create pos : StreamToken = {
        value = box { pos = pos }
        version = pos.index
        streamBytes = -1
    }

    let (|Unpack|) (token: StreamToken) : Position =
...
module Internal =
    ...
    type InternalSyncResult =
        ...
        | ConflictUnknown of StreamToken
        | Conflict of Position * ITimelineEvent<EventBody>[]

    [<RequireQualifiedAccess; NoComparison; NoEquality>]
    type LoadFromTokenResult<'event> =
        | Unchanged
        | Found of StreamToken * 'event[]

/// Defines the policies in force regarding how to split up calls when loading Event Batches via queries
type QueryOptions([<O; D null>] ?maxItems: int, [<O; D null>] ?getMaxItems: unit -> int, [<O; D null>] ?maxRequests)
...
type TipOptions(maxEvents, [<O; D null>] ?maxJsonLength, [<O; D null>] ?ignoreMissingEvents, [<O; D null>] ?readRetryPolicy, [<O; D null>] ?writeRetryPolicy)
...
type StoreClient(container: Container, archive: Container option, query: QueryOptions, tip: TipOptions) =
    ...
    let loadTip log stream pos =
        Tip.tryLoad log tip.ReadRetryPolicy (container, stream) (pos, None)
    ...
    member internal _.Read(log, stream, direction, (tryDecode, isOrigin), ?minIndex, ?maxIndex, ?tip) : Async<StreamToken * 'event[]> = async {
        ...
        let walk log container =
            Query.scan log (container, stream) includeTip query.MaxItems query.MaxRequests direction (tryDecode, isOrigin)
    ...
    member store.Load(log, (stream, maybePos), (tryDecode, isOrigin), checkUnfolds: bool) : Async<StreamToken * 'event[]> =
        if not checkUnfolds then
            store.Read(log, stream, Direction.Backward, (tryDecode, isOrigin))
        else
            async {
    ...
    member internal _.Sync(log, stream, exp, batch: Tip) : Async<InternalSyncResult> = async {
        if Array.isEmpty batch.e && Array.isEmpty batch.u then
...
type internal Category<'event, 'state, 'context>(store: StoreClient, codec: IEventCodec<'event, EventBody, 'context>) =
    ...
    member cat.Sync
        ...
        async {
            ...
            let renderElement =
                if compressUnfolds then
                    JsonElement.undefinedToNull >> JsonElement.deflate
                else
                    JsonElement.undefinedToNull

            let projections =
                projectionsEncoded |> Seq.map (Sync.mkUnfold renderElement baseIndex)
            ...
            | InternalSyncResult.Conflict(pos', tipEvents) ->
                return SyncResult.Conflict(fun ct -> cat.Reload(log, streamName, streamToken, state, fold, isOrigin, ct, (pos', pos.index, tipEvents)))
            | InternalSyncResult.ConflictUnknown _token' ->
                return SyncResult.Conflict(fun ct -> cat.Reload(log, streamName, streamToken, state, fold, isOrigin, ct))
...
module internal Caching =
    ...
    let applyCacheUpdatesWithSlidingExpiration (cache: ICache, prefix: string) (slidingExpiration: TimeSpan) =
        let mkCacheEntry struct (initialToken: StreamToken, initialState: 'state) =
    ...
    type CachingCategory<'event, 'state, 'context>
        (
            ...
            isOrigin: 'event -> bool,
            tryReadCache,
            updateCache: _ -> struct (_ * _) -> Task<unit>,
            checkUnfolds,
            compressUnfolds,
        ...
        interface ICategory<'event, 'state, 'context> with
            ...
            member _.TrySync
                ...
                task {
                    ...
                    match!
                        category.Sync(log, streamName, streamToken, state, events, mapUnfolds, fold, isOrigin, context, compressUnfolds)
                        |> Async.startAsTask ct
                    with
                    | SyncResult.Conflict resync -> return SyncResult.Conflict(fun ct -> cache streamName (fun () -> resync ct))
...
// Inhibits certificate verification when set to <c>true</c>, i.e. for working with the CosmosDB Emulator (default <c>false</c>)
 =
    ...
    member val Options =
        ...
        | Some ConnectionMode.Gateway
        | Some _ (* enum total match :( *)  -> co.ConnectionMode <- ConnectionMode.Gateway // only supports Https
        ...
        | x ->
            if co.ConnectionMode = ConnectionMode.Gateway then
                co.GatewayModeMaxConnectionLimit <- defaultArg x 50
        ...
        if bypassCertificateValidation = Some true && co.ConnectionMode = ConnectionMode.Gateway then
            ...
            let ch =
                new System.Net.Http.HttpClientHandler(ServerCertificateCustomValidationCallback = cb)
    ...
    member x.CreateUninitialized(discovery: Discovery) =
        discovery
        |> function
    ...
    member x.CreateAndInitialize(discovery: Discovery, containers) = async {
        ...
        | Discovery.AccountUriAndKey(accountUri = uri; key = key) ->
            return! CosmosClient.CreateAndInitializeAsync(string uri, key, containers, x.Options, ct) |> Async.AwaitTaskCorrect
 ...
 =
    ...
    let factory =
        CosmosClientFactory(
            ...
            ?mode = mode,
            ?gatewayModeMaxConnectionLimit = gatewayModeMaxConnectionLimit,
    ...
    /// Creates and validates a Client [including loading metadata](https://devblogs.microsoft.com/cosmosdb/improve-net-sdk-initialization) for the specified containers
    member _.CreateAndInitialize(containers) =
        factory.CreateAndInitialize(discovery, containers)
...
// Inhibit <c>CreateStoredProcedureIfNotExists</c> when a given Container is used for the first time
 =
    let createGateway =
        match createGateway with
        | Some creator -> creator
        | None -> id

    let primaryDatabaseAndContainerToArchive =
        defaultArg primaryDatabaseAndContainerToArchive id
    // Index of database*container -> Initialization Context
    let containerInitGuards =
        System.Collections.Concurrent.ConcurrentDictionary<string * string, Initialization.ContainerInitializerGuard>()
    ...
    // Container Name to use for locating missing events. Default: use <c>containerId</c>
     =
        ...
        let catAndStreamToDatabaseContainerStream (categoryName, streamId) =
            databaseId, containerId, genStreamName (categoryName, streamId)

        let primaryContainer (d, c) =
            (client: CosmosClient).GetDatabase(d).GetContainer(c)

        let fallbackContainer =
            if Option.isNone archiveClient && Option.isNone archiveDatabaseId && Option.isNone archiveContainerId then
                fun (_, _) -> None
            else
                fun (d, c) ->
                    Some(
                        (defaultArg archiveClient client)
                            .GetDatabase(defaultArg archiveDatabaseId d)
                            .GetContainer(defaultArg archiveContainerId c)
                    )

        CosmosStoreClient(
            catAndStreamToDatabaseContainerStream,
            primaryContainer,
            fallbackContainer,
    ...
    member internal _.ResolveContainerGuardAndStreamName(categoryName, streamId) : struct (Initialization.ContainerInitializerGuard * string) =
        let createContainerInitializerGuard (d, c) =
            let init =
                if Some true = disableInitialization then
                    None
                else
                    Some(fun cosmosContainer ct -> Initialization.createSyncStoredProcIfNotExists None cosmosContainer |> Async.Ignore |> Async.startAsTask ct)

            let archiveD, archiveC = primaryDatabaseAndContainerToArchive (d, c)

            let primaryContainer, fallbackContainer =
                createContainer (d, c), createFallbackContainer (archiveD, archiveC)
        ...
        let databaseId, containerId, streamName =
            categoryAndStreamNameToDatabaseContainerStream (categoryName, streamId)

        let g =
            containerInitGuards.GetOrAdd((databaseId, containerId), createContainerInitializerGuard)
...
type CosmosStoreContext(storeClient: CosmosStoreClient, tipOptions, queryOptions) =
    ...
    // Maximum number of trips to permit when slicing the work into multiple responses limited by `queryMaxItems`. Default: unlimited.
     =
        let tipOptions =
            TipOptions(maxEvents = tipMaxEvents, ?maxJsonLength = tipMaxJsonLength, ?ignoreMissingEvents = ignoreMissingEvents)

        let queryOptions =
            QueryOptions(?maxItems = queryMaxItems, ?maxRequests = queryMaxRequests)
    ...
    member internal x.ResolveContainerClientAndStreamIdAndInit(categoryName, streamId) =
        let struct (cg, streamName) =
            storeClient.ResolveContainerGuardAndStreamName(categoryName, streamId)
...
type CosmosStoreCategory<'event, 'state, 'context>(resolveInner, empty) =
    ...
    new(context: CosmosStoreContext, codec, fold, initial, caching, access, [<O; D null>] ?compressUnfolds)
     ...
     =
        ...
        let resolveCategory (categoryName, container) =
            let createCategory _name : ICategory<_, _, 'context> =
                ...
                Caching.CachingCategory<'event, 'state, 'context>(
                    cosmosCat,
                    fold,
                    initial,
                    isOrigin,
        ...
        let resolveInner struct (categoryName, streamId) =
            let struct (container, streamName, maybeContainerInitializationGate) =
                context.ResolveContainerClientAndStreamIdAndInit(categoryName, streamId)
...
type EventsContext internal (context: Equinox.CosmosStore.CosmosStoreContext, store: StoreClient, log: Serilog.ILogger)
// Logger to write to - see https://github.com/serilog/serilog/wiki/Provided-Sinks for how to wire to your logger
 =
    ...
    new(context: Equinox.CosmosStore.CosmosStoreContext, log) =
        let struct (store, _streamId, _init) =
            context.ResolveContainerClientAndStreamIdAndInit(null, null)

        EventsContext(context, store, log)

    member _.ResolveStream(streamName) =
        let struct (_cc, streamName, init) =
            context.ResolveContainerClientAndStreamIdAndInit(null, streamName)

        struct (streamName, init)

    member x.StreamId(streamName) : string =
        x.ResolveStream streamName |> ValueTuple.fst
...
module Events =
    ...
    let getBackwards (ctx: EventsContext) (streamName: string) (MaxPosition index: int64) (maxCount: int) : Async<ITimelineEvent<EventBody>[]> =
        ctx.Read(ctx.StreamId streamName, ?position = index, maxCount = maxCount, direction = Direction.Backward)
        |> dropPosition
