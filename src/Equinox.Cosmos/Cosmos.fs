...
module private Tip =
    ...
    let private loggedGet (get : CollectionStream * Position option -> Async<_>) (stream: CollectionStream, maybePos: Position option) (log: ILogger) = async {
        ...
        | ReadResult.NotModified ->
            (log 0 0 Log.TipNotModified).Information("Eqx {action:l} {res} {ms}ms rc={ru}", "Index", 302, (let e = t.Elapsed in e.TotalMilliseconds), ru)
        | ReadResult.NotFound ->
            (log 0 0 Log.TipNotFound).Information("Eqx {action:l} {res} {ms}ms rc={ru}", "Index", 404, (let e = t.Elapsed in e.TotalMilliseconds), ru)
 ...
 module private Query =
    ...
    let walk<'event> (log : ILogger) client retryPolicy maxItems maxRequests direction (stream: CollectionStream) startPos
            ...
            let! events =
                ...
                |> AsyncSeq.takeWhileInclusive (function
                    | x, Some e when isOrigin e ->
                        ...
                        | None -> log.Information("Eqx Stop stream={stream} at={index} {case}", stream.name, x.Index, x.EventType)
                        | Some batch ->
                            let used, residual = batch |> calculateUsedVersusDroppedPayload x.Index
                            log.Information("Eqx Stop stream={stream} at={index} {case} used={used} residual={residual}",
        ...
        log |> logBatchRead direction maxItems stream.name t (!responseCount,raws) pos.index ru
        return pos, decoded }
