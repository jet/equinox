...
module private Histograms =
    ...
    let labelNames tagNames =
        Array.append tagNames [| "rut"; "facet"; "op"; "db"; "con"; "cat" |]

    let labelValues tagValues (rut, facet, op, db, con, cat) =
        Array.append tagValues [| rut; facet; op; db; con; cat |]
    ...
    let private mkHistogram (cfg: Prometheus.HistogramConfiguration) name desc =
        ...
        fun tagValues (rut, facet: string, op: string) (db, con, cat: string) s -> h.WithLabels(labelValues tagValues (rut, facet, op, db, con, cat)).Observe(s)
...
module private Summaries =
    ...
    let labelNames tagNames =
        Array.append tagNames [| "facet"; "db"; "con" |]

    let labelValues tagValues (facet, db, con) =
        Array.append tagValues [| facet; db; con |]
...
module private Counters =
    ...
    let labelNames tagNames =
        Array.append tagNames [| "facet"; "op"; "outcome"; "db"; "con"; "cat" |]

    let labelValues tagValues (facet, op, outcome, db, con, cat) =
        Array.append tagValues [| facet; op; outcome; db; con; cat |]
    ...
    let eventsAndBytesPair tags stat desc =
        ...
        fun ctx (db, con, cat, e, b) ->
            ...
            match b with
            | None -> ()
            | Some b -> observeB ctx (db, con, cat) b
