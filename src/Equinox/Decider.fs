...
type Decider<'event, 'state>(stream: IStream<'event, 'state>) =
    ...
    member _.Transact(decide: 'state -> 'result * 'event list, ?load, ?attempts) : Async<'result> = async {
        ...
        let inline decide' s =
            let r, es = decide s in struct (r, Seq.ofList es)
    ...
    member _.TransactEx(decide: ISyncContext<'state> -> 'result * 'event list, ?load, ?attempts) : Async<'result> = async {
        ...
        let inline decide' c =
            let r, es = decide c in struct (r, Seq.ofList es)
    ...
    member _.TransactEx
        (
            decide: ISyncContext<'state> -> 'result * 'event list,
            mapResult: 'result -> ISyncContext<'state> -> 'view,
            ?load,
            ?attempts
        ) : Async<'view> =
        async {
    ...
    member _.TransactAsync(interpret: 'state -> Async<'event list>, render: 'state -> 'view, ?load, ?attempts) : Async<'view> = async {
        ...
        let inline interpret' s ct = task {
            let! es = Async.StartImmediateAsTask(interpret s, ct)
            return Seq.ofList es
        }
    ...
    member _.TransactAsync(decide: 'state -> Async<'result * 'event list>, ?load, ?attempts) : Async<'result> = async {
        ...
        let inline decide' s ct = task {
            let! r, es = Async.StartImmediateAsTask(decide s, ct)
            return struct (r, Seq.ofList es)
        }
    ...
    member _.TransactExAsync(decide: ISyncContext<'state> -> Async<'result * 'event list>, ?load, ?attempts) : Async<'result> = async {
        ...
        let decide' c ct = task {
            let! r, es = Async.StartImmediateAsTask(decide c, ct)
            return struct (r, Seq.ofList es)
        }
...
and DeciderCore<'event, 'state>(stream: IStream<'event, 'state>) =
    ...
    /// 2. (if events yielded) Attempt to sync the yielded events to the stream.
    ///    (Restarts up to <c>maxAttempts</c> times with updated state per attempt, throwing <c>MaxResyncsExhaustedException</c> on failure of final attempt.)
    member _.Transact(interpret: Func<'state, 'event seq>, [<O; D null>] ?load, [<O; D null>] ?attempts, [<O; D(CancellationToken())>] ?ct) : Task<unit> =
        let inline decide struct (_t: StreamToken, state) _ct =
            Task.FromResult struct ((), interpret.Invoke state)
    ...
    member _.Transact
        (
            interpret: Func<'state, 'event seq>,
            render: Func<'state, 'view>,
            [<O; D null>] ?load,
            [<O; D null>] ?attempts,
            [<O; D(CancellationToken())>] ?ct
        ) : Task<'view> =
        let inline decide struct (_token, state) _ct =
            Task.FromResult struct ((), interpret.Invoke state)
    ...
    ///    (Restarts up to <c>maxAttempts</c> times with updated state per attempt, throwing <c>MaxResyncsExhaustedException</c> on failure of final attempt.)
    /// 3. Yield result
    member _.Transact(decide: Func<'state, struct ('result * 'event seq)>, [<O; D null>] ?load, [<O; D null>] ?attempts, [<O; D null>] ?ct) : Task<'result> =
    ...
    member _.Transact
        (
            decide: Func<'state, struct ('result * 'event seq)>,
            mapResult: Func<'result, 'state, 'view>,
            [<O; D null>] ?load,
            [<O; D null>] ?attempts,
            [<O; D null>] ?ct
        ) : Task<'view> =
    ...
    member _.TransactEx
        (
            decide: Func<ISyncContext<'state>, struct ('result * 'event seq)>,
            [<O; D null>] ?load,
            [<O; D null>] ?attempts,
            [<O; D null>] ?ct
        ) : Task<'result> =
        let inline decide' (Context c) _ct =
            let r = decide.Invoke(c) in Task.FromResult r
    ...
    member _.TransactEx
        (
            decide: Func<ISyncContext<'state>, struct ('result * 'event seq)>,
            mapResult: Func<'result, ISyncContext<'state>, 'view>,
            [<O; D null>] ?load,
            [<O; D null>] ?attempts,
            [<O; D null>] ?ct
        ) : Task<'view> =
    ...
    ///    (Restarts up to <c>maxAttempts</c> times with updated state per attempt, throwing <c>MaxResyncsExhaustedException</c> on failure of final attempt.)
    /// 3. Uses <c>render</c> to generate a 'view from the persisted final state
    member _.TransactAsync
        (
            interpret: Func<'state, CancellationToken, Task<'event seq>>,
            render: Func<'state, 'view>,
            [<O; D null>] ?load,
            [<O; D null>] ?attempts,
            [<O; D null>] ?ct
        ) : Task<'view> =
        let inline decide struct (_token, state) ct = task {
            let! es = interpret.Invoke(state, ct)
            return struct ((), es)
        }
    ...
    member _.TransactAsync
        (
            decide: Func<'state, CancellationToken, Task<struct ('result * 'event seq)>>,
            [<O; D null>] ?load,
            [<O; D null>] ?attempts,
            [<O; D null>] ?ct
        ) : Task<'result> =
        let inline decide struct (_token, state) ct = task {
            let! r, e = decide.Invoke(state, ct)
            return struct (r, e)
        }
    ...
    member _.TransactExAsync
        (
            decide: Func<ISyncContext<'state>, CancellationToken, Task<struct ('result * 'event seq)>>,
            [<O; D null>] ?load,
            [<O; D null>] ?attempts,
            [<O; D null>] ?ct
        ) : Task<'result> =
        let inline decide (Context c) ct = task {
            let! r, e = decide.Invoke(c, ct)
            return struct (r, e)
        }
...
and internal LoadPolicy() =
    static member Fetch<'state, 'event>(x: LoadOption<'state> option) : IStream<'event, 'state> -> CancellationToken -> Task<struct (StreamToken * 'state)> =
        match x with
        | None
        | Some RequireLoad -> fun stream ct -> stream.Load(allowStale = false, requireLeader = false, ct = ct)
...
and [<NoComparison; NoEquality; RequireQualifiedAccess>] Attempts = Max of count: int

and internal AttemptsPolicy() =
    static member Validate(opt: Attempts option) =
        let maxAttempts =
            match opt with
            | Some(Attempts.Max n) -> n
            | None -> 3

        if maxAttempts < 1 then
            raise (ArgumentOutOfRangeException(nameof opt, maxAttempts, "should be >= 1"))

        fun attempt ->
            if attempt = maxAttempts then
                raise (MaxResyncsExhaustedException attempt)
...
and internal SyncContext<'state> =
    ...
    static member Map(struct (token: StreamToken, state: 'state)) =
        { new ISyncContext<'state> with
            ...
            member _.StreamEventBytes =
                match token.streamBytes with
                | -1L -> ValueNone
                | b -> ValueSome b
