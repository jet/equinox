...
type Decider<'event, 'state>(inner : DeciderCore<'event, 'state>) =
    ...
    ///    (Restarts up to <c>maxAttempts</c> times with updated state per attempt, throwing <c>MaxResyncsExhaustedException</c> on failure of final attempt.)
    /// 3. Uses <c>render</c> to generate a 'view from the persisted final state
    member _.TransactAsync(interpret : 'state -> CancellationToken -> Task<'event seq>, render : 'state -> 'view, ?load, ?attempts, ?ct) : Task<'view> =  task {
        let interpret' = Func<_,_,_>(interpret)
    ...
    ///    (Restarts up to <c>maxAttempts</c> times with updated state per attempt, throwing <c>MaxResyncsExhaustedException</c> on failure of final attempt.)
    /// 3. Yield result
    member _.TransactAsync(decide : 'state -> CancellationToken -> Task<struct('result * 'event seq)>, ?load, ?attempts, ?ct) : Task<'result> =  task {
        let decide' = Func<_,_,_>(decide)
        return! inner.TransactAsync(decide = decide', CancellationToken, Task<'result * 'event seq>>(decide), ?load = load, ?attempts = attempts, ?ct = ct) }

    /// 1. Invoke the supplied <c>Task</c> returning <c>decide</c> function with the current complete context, holding the <c>'result</c>
    /// 2. (if events yielded) Attempt to sync the yielded events to the stream.
    ///    (Restarts up to <c>maxAttempts</c> times with updated state per attempt, throwing <c>MaxResyncsExhaustedException</c> on failure of final attempt.)
    /// 3. Yield result
    member _.TransactExAsync(decide : ISyncContext<'state> -> CancellationToken -> Task<struct('result * 'event seq)>, ?load, ?attempts, ?ct) : Task<'result> =  task {
        let decide' = Func<_,_,_>(decide)
    ...
    ///    (Restarts up to <c>maxAttempts</c> times with updated state per attempt, throwing <c>MaxResyncsExhaustedException</c> on failure of final attempt.)
    /// 3. Yields a final 'view produced by <c>mapResult</c> from the <c>'result</c> and/or the final persisted <c>ISyncContext</c>
    member _.TransactExAsync(decide : ISyncContext<'state> -> CancellationToken -> Task<struct('result * 'event seq)>, mapResult : 'result -> ISyncContext<'state> -> 'view,
                             ?load, ?attempts, ?ct) : Task<'view> =  task {
        let decide' = Func<_,_,_>(decide)
        return! inner.TransactExAsync(decide = decide', mapResult = mapResult, ?load = load, ?attempts = attempts, ?ct = ct) }
