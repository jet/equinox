...
and [<Struct; NoEquality; NoComparison>] StreamToken = {
    value: obj
    version: int64
    streamBytes: int64
}
...
type internal Impl() =
    ...
    static let run
        ...
        let rec loop attempt tokenAndState : Task<'v> = task {
            ...
            | events ->
                ...
                | SyncResult.Written tokenAndState' -> return mapResult.Invoke(result, tokenAndState')
    ...
    static member TransactAsync
        (
            stream,
            fetch: IStream<'e, 's> -> CancellationToken -> Task<struct (StreamToken * 's)>,
            decide,
            reload,
            mapResult,
            ct
    ...
    static member QueryAsync
        (
            stream,
            fetch: IStream<'e, 's> -> CancellationToken -> Task<struct (StreamToken * 's)>,
            projection: Func<struct (StreamToken * 's), 'v>,
            ct
        ) : Task<'v> =
        task {
