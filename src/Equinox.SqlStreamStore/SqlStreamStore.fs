...
module private Write =
    ...
    let private writeEventsAsync
        ...
        async {
            ...
            with :? WrongExpectedVersionException as ex ->
                log.Information(
                    ex,
                    "SqlEs TrySync WrongExpectedVersionException writing {EventTypes}, expected {ExpectedVersion}",
    ...
    let eventDataBytes events =
        let eventDataLen (x: NewStreamMessage) =
            match x.JsonData |> System.Text.Encoding.UTF8.GetBytes, x.JsonMetadata |> System.Text.Encoding.UTF8.GetBytes with
    ...
    let private writeEventsLogged
        ...
        async {
            ...
            let resultLog, evt =
                ...
                | EsSyncResult.ConflictUnknown, m -> log, Log.WriteConflict m
            ...
            (resultLog |> Log.event evt)
                .Information(
                    "SqlEs{action:l} count={count} conflict={conflict}",
                    "Write",
                    events.Length,
                    match evt with
...
module private Read =
    ...
    let (|ResolvedEventLen|) (x: StreamMessage) =
        ...
        match data, x.JsonMetadata with
        | Log.StrLen bytes, Log.StrLen metaBytes -> bytes + metaBytes
    ...
    let private loggedReadSlice conn streamName direction batchSize startPos (log: ILogger) : Async<ReadStreamPage> = async {
        ...
        let bytes, count =
            slice.Messages |> Array.sumBy (|ResolvedEventLen|), slice.Messages.Length
        ...
        let log =
            if (not << log.IsEnabled) Events.LogEventLevel.Debug then
                ...
                log |> Log.propResolvedEvents "Json" slice.Messages

        (log |> Log.prop "startPos" startPos |> Log.prop "bytes" bytes |> Log.event evt)
            .Information("SqlEs{action:l} count={count} version={version}", "Read", count, slice.LastStreamVersion)
            ...
            | PageReadStatus.Success ->
                let version =
                    if batchCount = 0 then
                        Some(int64 slice.LastStreamVersion)
                    else
                        None
    ...
    let logBatchRead direction streamName t events batchSize version (log: ILogger) =
        ...
        (log |> Log.prop "bytes" bytes |> Log.event evt)
            .Information("SqlEs{action:l} stream={stream} count={count}/{batches} version={version}", action, streamName, count, batches, version)
    ...
    let partitionPayloadFrom firstUsedEventNumber : ResolvedEvent[] -> int * int =
        let acc (tu, tr) (ResolvedEventLen bytes as y) =
            if y.Position < firstUsedEventNumber then
                tu, tr + bytes
            else
                tu + bytes, tr
                        ...
                        |> AsyncSeq.takeWhileInclusive (function
                            | x, ValueSome e when isOrigin e ->
                                ...
                                | Some batch ->
                                    ...
                                    log.Information(
                                        "SqlEsStop stream={stream} at={eventNumber} used={used} residual={residual}",
                                        streamName,
                                        x.Position,
                                        used,
                                        residual
...
module UnionEncoderAdapters =
    ...
    let (|Bytes|) =
        function
        | null -> null
        | (s: string) -> System.Text.Encoding.UTF8.GetBytes s

    let encodedEventOfResolvedEvent (e: StreamMessage) : FsCodec.ITimelineEvent<EventBody> =
        let (Bytes data) =
            e.GetJsonData() |> Async.AwaitTaskCorrect |> Async.RunSynchronously
    ...
    let eventDataOfEncodedEvent (x: FsCodec.IEventData<EventBody>) =
        ...
        let mapData (x: EventBody) =
            if x.IsEmpty then
                "{}"
            else
                System.Text.Encoding.UTF8.GetString(x.Span)

        let mapMeta (x: EventBody) =
            if x.IsEmpty then
                null
            else
                System.Text.Encoding.UTF8.GetString(x.Span)
...
type SqlStreamStoreContext(connection: SqlStreamStoreConnection, batchOptions: BatchOptions) =
    ...
    new(connection: SqlStreamStoreConnection, [<O; D null>] ?batchSize)
    ...
    member internal _.LoadBatched(streamName, requireLeader, log, tryDecode, isCompactionEventType) : Async<StreamToken * 'event[]> = async {
        let! version, events =
    ...
    member internal _.LoadBackwardsStoppingAtCompactionEvent(streamName, requireLeader, log, tryDecode, isOrigin) : Async<StreamToken * 'event[]> = async {
        let! version, events =
    ...
    member internal _.Reload
        ...
        ) : Async<StreamToken * 'event[]> =
        async {
            let streamPosition = token.streamVersion + 1L

            let! version, events =
                Read.loadForwardsFrom
                    log
                    connection.ReadRetryPolicy
                    (conn requireLeader)
                    batchOptions.BatchSize
    ...
    member internal _.TrySync
        (
            log,
            streamName,
            (Token.Unpack pos as streamToken),
            events,
            encodedEvents: EventData array,
            isCompactionEventType
        ...
        async {
            ...
            | EsSyncResult.ConflictUnknown -> return GatewaySyncResult.ConflictUnknown
...
type private Category<'event, 'state, 'context>
    (
        context: SqlStreamStoreContext,
        codec: FsCodec.IEventCodec<_, _, 'context>,
            ...
            | GatewaySyncResult.ConflictUnknown ->
                return
                    SyncResult.Conflict(fun ct ->
                        load fold state (context.Reload(streamName (*requireLeader*) , true, log, streamToken, tryDecode, compactionPredicate))
...
type private Folder<'event, 'state, 'context>(category: Category<'event, 'state, 'context>, fold: 'state -> 'event seq -> 'state, initial: 'state, ?readCache) =
    let batched log streamName requireLeader ct =
        category.Load(fold, initial, streamName, requireLeader, log) |> Async.startAsTask ct
...
type SqlStreamStoreCategory<'event, 'state, 'context>(resolveInner, empty) =
    ...
    new(context: SqlStreamStoreContext, codec: FsCodec.IEventCodec<_, _, 'context>, fold, initial, [<O; D(null)>] ?caching, [<O; D(null)>] ?access)
...
type ConnectorBase([<O; D(null)>] ?readRetryPolicy, [<O; D(null)>] ?writeRetryPolicy) =
    ...
    member x.Establish() : Async<SqlStreamStoreConnection> = async {
        ...
        return
            SqlStreamStoreConnection(readConnection = store, writeConnection = store, ?readRetryPolicy = readRetryPolicy, ?writeRetryPolicy = writeRetryPolicy)
    }
