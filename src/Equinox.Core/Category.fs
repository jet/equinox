...
type ICategory<'event, 'state, 'context> =
    ...
    abstract Load:
        log: ILogger * categoryName: string * streamId: string * streamName: string * allowStale: bool * requireLeader: bool * ct: CancellationToken ->
            ...
            member _.TrySync(attempt, (token, originState), events, ct) =
                let log =
                    if attempt = 1 then
                        ...
                        log.ForContext("attempts", attempt)
...
module Stream =
    ...
    let resolveWithContext (ctx: 'context) log (cat: Category<'event, 'state, 'context>) : struct (string * string) -> Core.IStream<'event, 'state> =
        fun struct (categoryName, streamId) -> cat.Stream(log, ctx, categoryName, streamId)

    let resolve log (cat: Category<'event, 'state, unit>) = resolveWithContext () log cat
...
module Decider =
    ...
    let resolveCore log (cat: Category<'event, 'state, unit>) = resolveCoreWithContext () log cat
...
type DeciderExtensions =
    ...
    static member Resolve
        (
            cat: Category<'event, 'state, 'context>,
            context: 'context,
            log
        ) : System.Func<struct (string * string), DeciderCore<'event, 'state>> =
