...
type CacheEntry<'state>(initialToken: StreamToken, initialState: 'state) =
    ...
    member x.Value: struct (StreamToken * 'state) =
        lock x <| fun () -> currentToken, currentState

type ICache =
    abstract member UpdateIfNewer:
        key: string * options: CacheItemOptions * (struct (StreamToken * StreamToken) -> bool) * entry: CacheEntry<'state> -> Task<unit>
    ...
    interface ICache with
        member _.UpdateIfNewer(key, options, supersedes, entry) =
            ...
            | :? CacheEntry<'state> as existingEntry ->
                existingEntry.UpdateIfNewer(supersedes, entry)
                Task.FromResult()
