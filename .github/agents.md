...
- CI-only pack behavior is enabled when `CI=true`; packaged projects under `src/` and `tools/` opt into documentation generation and package validation from their local `Directory.Build.props`.
