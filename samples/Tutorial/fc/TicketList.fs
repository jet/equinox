...
module Folds =
    ...
    let evolve state = function
        | Events.Allocated e -> (state,e.tickets) ||> Array.fold (fun m x -> Set.add x m)
        | Events.Snapshotted e -> Set.ofArray e.tickets
    let fold : State -> Events.Event seq -> State = Seq.fold evolve
    let isOrigin = function Events.Snapshotted _ -> true | Events.Allocated _ -> false
    let snapshot state = Events.Snapshotted { tickets = Set.toArray state }
