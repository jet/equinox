...
let genCodec<'Union when 'Union :> TypeShape.UnionContract.IUnionContract>(utf8Json) =
    if utf8Json then
        Equinox.Codec.JsonNet.JsonUtf8.Create<'Union>(serializationSettings)
    else
        Equinox.Codec.Utf8Json.JsonUtf8.Create<'Union>()
...
type ServiceBuilder(storageConfig, handlerLog, utf8json) =
     let resolver = StreamResolver(storageConfig)

     member __.CreateFavoritesService() =
        let codec = genCodec<Domain.Favorites.Events.Event>(utf8json)
     ...
     member __.CreateSaveForLaterService() =
        let codec = genCodec<Domain.SavedForLater.Events.Event>(utf8json)
     ...
     member __.CreateTodosService() =
        let codec = genCodec<TodoBackend.Events.Event>(utf8json)
...
let register (services : IServiceCollection, storageConfig, handlerLog, utf8json) =
    let regF (factory : IServiceProvider -> 'T) = services.AddSingleton<'T>(fun (sp: IServiceProvider) -> factory sp) |> ignore

    regF <| fun _sp -> ServiceBuilder(storageConfig, handlerLog, utf8json)
