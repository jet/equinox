...
type Store(store) =
    member _.Category
        ...
        | Storage.StorageConfig.MsgDb(context, caching) ->
            Equinox.MessageDb.MessageDbCategory<'event,'state,_>(context, codec.ToJsonElementCodec(), fold, initial, ?caching = caching)
