...
module Folds =
    ...
    and Stats =
        {   requested   : int
            ...
            releasing   : int
            assigned    : int list }
    let (|Idle|Running|Canceling|) = function NotStarted | Completed -> Idle | Running s -> Running s | Canceling s -> Canceling s
    module States =
        let (|ToSet|) = set
    ...
    let evolve state = function
        ...
        | Events.Assigned e ->
            ...
            | Canceling s ->    Canceling (s |> States.withAssigned e.listId)
        | Events.Revoked e ->
            match state with
            | Idle ->           failwith "Cannot have Released if Idle"
            | Running s ->      Running (s |> States.withRevoked e.ticketIds)
            | Canceling s ->    Canceling (s |> States.withRevoked e.ticketIds)
        | Events.Completed ->
            match state with
            | Running s
            | Canceling s when Set.isEmpty s.unknown && Set.isEmpty s.reserved && List.isEmpty s.assigning ->
                                Completed
