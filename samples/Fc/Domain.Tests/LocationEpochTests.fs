...
let [<Property>] properties carriedForward delta1 closeImmediately delta2 close =
    ...
    let res,events = sync (Some carriedForward) (Some (validateAndInterpret carriedForward delta1)) initialShouldClose Folds.initial
    ...
    let { isOpen = isOpen; worked = worked; balance = bal },events = sync None (Some (validateAndInterpret expectedBalance delta2)) shouldClose state1
