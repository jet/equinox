...
module Cart =
    ...
    let createServiceWithCompaction log gateway =
        let resolveStream = GesStreamBuilder(gateway, codec, fold, initial, AccessStrategy.RollingSnapshots compact).Create
...
module ContactPreferences =
    ...
    let createService log connection =
        let resolveStream = GesStreamBuilder(createGesGateway connection 1, codec, fold, initial, AccessStrategy.EventsAreState).Create
