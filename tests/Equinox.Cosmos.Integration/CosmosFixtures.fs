...
let private connectToCosmos (log: Serilog.ILogger) name discovery =
    Connector(log=log, requestTimeout=TimeSpan.FromSeconds 3., maxRetryAttemptsOnThrottledRequests=2, maxRetryWaitTimeInSeconds=60, mode=Microsoft.Azure.Cosmos.ConnectionMode.Direct)
