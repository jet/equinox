...
- Events are stored in immutable documents in the logical partition, the `Tip` document is the only document that's ever updated (it's _always_ updated, as it happens...).
...
| `Snapshot` | 1) read Tip; stop if `isOrigin` accepts a snapshot from within <br/> 2) read backwards until `isOrigin` likes an event, or we hit start of stream | 1) Produce proposed `state'` <br/> 2) write events to new document + `toSnapshot state'` result into Tip with new event count |
