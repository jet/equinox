...
- The `unfolds` maintained in `Tip` have the bodies (the `d` and `m` fields) 1) deflated 2) base64 encoded (as everyone is reading the Tip, its worthwhile having the writer take on the burden of compressing, with the payback being that write amplification effects are reduced by readers paying less RUs to read them). The snapshots can be inspected securely via the `eqx` tool's `dump` facility, or _unsecurely_ online via the [**decode** button on `jgraph`'s `drawio-tools`(https://jgraph.github.io/drawio-tools/tools/convert.html), _if the data is not sensitive_](https://jgraph.github.io/drawio-tools/tools/convert.html).
